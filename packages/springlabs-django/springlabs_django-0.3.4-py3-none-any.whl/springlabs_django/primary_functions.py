...
def modifyFilesNewGraph(version,group,name,type_graph,project_name):
    ...
    errors = []
    primary_package = "springlabs_django"
    SPRINGLABS_DJANGO_JSON = "springlabs_django.json"
    

    graph_version = version
    group_graph = group
    try:
        with open(SPRINGLABS_DJANGO_JSON) as file:
            data = json.load(file)
    except:
        message = f"Ocurrió un problema interno al leer archivo ({SPRINGLABS_DJANGO_JSON})"
    ...
    if delete_example==True:
        ...
        try:
            ...
            message = f"Ocurrio un error interno al actualizar archivo {SPRINGLABS_DJANGO_JSON}"
            errors.append([message, True])
        else:
            versions_detail = data['versions_detail']
            # Recorremos todas las versiones
            ...
            try:
                with open(SPRINGLABS_DJANGO_JSON, "w") as file:
