...
"""
Functions to simulate free-flow movement of vehicle's.
"""


def gear_for_speed_profiles(gs, curr_speed, current_gear, gear_cnt, sim_step=0.1, clutch_duration=0.5):
    """
    ...
    if gear_limits[current_gear - 1] - gear_limits[current_gear - 1] * downshift_off <= curr_speed < \
        ...
        while itr == 1:
            if gear_limits[gear_search - 1] <= curr_speed < gear_limits[gear_search]:
                gear_cnt = int(divmod(10 * clutch_duration, 10 * sim_step)[0])  # in simulation steps for 0.5 second
...
def accMFC(velocity, driver_style, desired_velocity, acc_p_curve, dec_p_curve):
    """
    ...
    :param acc_p_curve:
        Acceleration potential curve of the gear in use.
    :type acc_p_curve:

    :param dec_p_curve:
        Deceleration potential curve of the gear in use.
    :type dec_p_curve:

    :return:
    """
    if velocity >= desired_velocity:
        on_off = (1 - pow(1 - (velocity - desired_velocity) / 50, 100))
        acc = dec_p_curve(velocity) * driver_style * on_off
