...
def simple_run():
    ...
    plt.text(30, 22, f'Simulation car:{car_id}, DS:{driver_style}, GS:{gs_style}')
    ...
    plt.plot(Speeds[1:], Acceleration[1:], 'k', label='simulation')
    ...
    fig4 = plt.figure('Acceleration over time')
    plt.plot(times, Acceleration[1:])
    plt.text(23, 2.2, f'Simulation car:{car_id}, DS:{driver_style}, GS:{gs_style}')
    ...
    for gear, gear_curve in enumerate(Curves, start=1):
        sp_bins = np.arange(StartStop[0][gear - 1], StartStop[1][gear - 1] + 0.1, 0.1)
        accelerations = gear_curve(sp_bins)
        plt.plot(sp_bins, accelerations, label=f'gear {gear}')
    plt.legend()
    plt.text(23, 2.5, f'Simulation car:{car_id}, DS:{driver_style}, GS:{gs_style}')
