...
to change a single line of code. This gets achieved within
...
So for to add help support to the widgets of your app you only
need to add/provide the help texts with a help id that is
matching the value of the :attr:`ae_help_id` attribute of
the widget you need help for.

Additionally you can provide a separate i18n translation message
file for each of the supported languages for to make your
...
So :attr:`~HelpAppBase.help_layout is also used as a flag
...
simply add a `count` item to the `ae_help_vars`
...
HELP_ID_PREFIX_FLOW = 'help_flow#'
HELP_ID_PREFIX_STATE = 'help_app_state#'
...
class HelpAppBase(MainAppBase, ABC):
    ...
    def help_display(self, help_id: str, help_vars: Dict[str, Any], widget: Optional[Any] = None,
                     key_suffix: str = '', must_have: bool = False) -> bool:
        ...
        wid = None
        if help_id:     # if not in init/reset then ensure target widget and extend help_vars with call stack locals
            wid = self.help_widget('ae_help_id', help_id, help_vars)
            if not wid:
                wid = self.widget_by_attribute('ae_help_id', help_id)
                if wid:
                    help_vars['self'] = wid
                elif widget and widget not in (wid, self.help_activator):
                    self.dpo(f"HelpAppBase.help_display(): target={wid}/{help_id} missing or IS NOT event_wid={widget}")
        widget = wid or widget or self.help_activator
        ...
        glo_vars['_add_base_globals'] = True
    ...
    def help_widget(self, attr_name: str, attr_value: str, help_vars: Dict[str, Any]) -> Optional[Any]:
        """ find help target widget via attribute name/value and extend :paramref:`~help_widget.help_vars`.

        :param attr_name:       widget attribute name for to detect widget and call stack locals.
        :param attr_value:      widget attribute value for to detect widget and call stack locals
