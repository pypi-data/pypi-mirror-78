...
class PipeNonOpt(SimEnv):
    ...
    def __init__(self, name, master_cls, **kwargs):
        ...
        self._lam_T = np.zeros(self.num_gp)  # , self._tiny, dtype=np.float64)
        ...
        self._cp_T = np.zeros(self.num_gp)  # , self._tiny, dtype=np.float64)
        ...
        self._rho_T = np.zeros(self.num_gp)  # , self._tiny, dtype=np.float64)
        self._rhocp = np.zeros_like(self._rho_T)  # volume specific heat cap.
        # preallocate kinematic viscosity grid:
        self._ny_T = np.zeros(self.num_gp)  # , self._tiny, dtype=np.float64)
        ...
        self.dm = np.zeros_like(self.T)  # general massflow array
        self._dm_cell = np.zeros_like(self.T)  # array for cell center flow
        self._dm_top = np.zeros_like(self.T)  # array for pos. flow from top
        self._dm_bot = np.zeros_like(self.T)  # array for pos. flow from bottom
        self._dm_port = np.zeros_like(self.T)  # array for inflow through prts
    ...
    def init_part(self, **kwargs):
        ...
        err_str = (
            'The insulation thickness of a pipe has '
        ...
        err_str = (
            '`pipe_specs` and `material` for pipe '
            + self.name
        ...
        err_str = (
            'For a part of type pipe, the same `pipe_specs` must be '
        ...
        err_str = (
            ...
            'to the `init_part()` method of part ' + self.name + ' as '
    ...
    def get_diff(self, timestep):
        ...
        # ---> get massflows
        #        if self.dm >= 0:  # massflow from the top
        #            # get massflow though top cell-cell border:
        #            self._dm_top[1:] = self.dm
        #            self._dm_bot[:-1] = 0  # set massflow from the bottom to zero
        #        else:  # massflow from the bottom:
        #            # get massflow though bottom cell-cell border (and *-1!):
        #            self._dm_bot[:-1] = - self.dm
        #            self._dm_top[1:] = 0  # set massflow from the top to zero
        #        # get ports massflow (only for the positive inflow):
        #        if self._dm_io >= 0:
        #            self._dm_port[0] = self._dm_io
        #            self._dm_port[-1] = 0
        #        else:
        #            self._dm_port[-1] = - self._dm_io
        #            self._dm_port[0] = 0
        #
        #        # save to result massflow grid
        #        self.res_dm[self._models.stepnum] = self.dm[0]
        ...
        #        self._lam_fld_own_p[self._port_own_idx] = self._lam_T[self._port_own_idx]
        ...
        _pf._lambda_mean(lam_T=self._lam_T, out=self._lam_mean)
        ...
        #        # for conduction between current cell and cells at ports (A_p_fld_mean
        #        # is 0 at ports where no cond. shall be calculated to avoid loss):
        ...
        dT_cond_port = _pf._process_ports(
        ...
        # #                self._trnc_err_cell_weight *= 1.1
        #                self._models._max_factor = self._models._max_factor**0.99
        #                self._models._max_factor = 1.05 if self._models._max_factor < 1.05 else self._models._max_factor
        #            else:
        ...
        self.dT_cond[0] += dT_cond_port[0]
        self.dT_cond[-1] += dT_cond_port[-1]
