usage: minmon [-h] [-V] [-s SEC] [-t] [-n] [-g] [-c] [-l LOG]
...
```
   ...
   -t, --time-only     display time only (default: date and time, no effect on CSV format)
   -n, --numeric-only  display numeric only (default: numeric and graphic, no effect on CSV format)
   -g, --graphic-only  display graphic only (default: numeric and graphic, no effect on CSV format)
   -c, --csv           output in CSV format
   -l LOG, --log LOG   write lines into LOG logfile (default path: '~/.minmon')
```