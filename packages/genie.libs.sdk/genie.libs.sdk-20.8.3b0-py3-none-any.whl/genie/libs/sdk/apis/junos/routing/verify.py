...
def verify_route_has_as_path(
    ...
    invert: bool = False,
    origin_route: bool = False,
    non_extensive: bool = False,
) -> bool:
    ...
    Args:
        ...
        origin_route (bool, optional): True if target route needs to be verified. Defaults to False.
        non_extensive (bool, optional): True if show command not ends with 'extensive'. Defaults to False. 
    ...
    while timeout.iterate():
        ...
        try:
            ...
            if origin_route:
                out = device.parse("show route protocol bgp extensive")
            elif not non_extensive:
                out = device.parse(
                    "show route protocol bgp {route} extensive".format(route=target_route))
        except SchemaEmptyParserError:
            timeout.sleep()
            continue
        ...
        for route_table in out.q.get_values('route-table'):
            for rt in Dq(route_table).get_values('rt'):
                rt_destination = rt.get('rt-destination', None)

                if origin_route and rt_destination != target_route:
                    continue
                try:
                    ...
                    as_paths_ = rt.get('tsi', {}).get('#text', '').split("\n")
                    expected_as_path = "AS path: {}".format(str(expected_as_path))
                if not op(as_paths_, str(expected_as_path)):
                    continue
                return True
...
def verify_route_as_path_count(
    ...
    while timeout.iterate():
        try:
            out = device.parse('show route protocol bgp {route}'.format(
                route=route
            ))
        except SchemaEmptyParserError:
            timeout.sleep()
        ...
        for rt in out.q.get_values('rt'):
            if best_path and rt.get('rt-entry', {}).get('active-tag') == "*":
                as_path_ = rt.get('rt-entry', {}).get('as-path')
                as_path = [path for path in as_path_.split() if path.translate(
                    {ord('('):None,ord(')'):None}) not in excluded_paths]
                break
            elif not best_path and not rt.get('rt-entry', {}).get('active-tag'):
                as_path_ = rt.get('rt-entry', {}).get('as-path')
                as_path = [path for path in as_path_.split() if path.translate(
                    {ord('('):None,ord(')'):None}) not in excluded_paths]
                break
