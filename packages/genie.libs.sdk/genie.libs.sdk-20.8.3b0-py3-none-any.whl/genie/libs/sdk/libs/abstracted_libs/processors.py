...
def delete_configuration(section, devices):
    ...
    Args:
        ...
        Parameters: Parameters passed from trigger data file.
            devices (`list`): List of devices from sections
                templates_dir (`str`): Templates directory. Default to None.
                template_name (`str`): Template name. Default to None.
                    jinja2_parameters (`dict`): Key/Value pair for jinja template arguments
    ...
    for name, dev in devices.items():
        ...
        try:
            ...
            templates_dir = dev['templates_dir']
            template_name = dev['template_name']
            log.info("loading config file {}/{}".format(templates_dir,template_name))
            
            # Initialize Jinja2 parameters
            jinja2_parameters = dev.get('jinja2_parameters', {}).copy()
            jinja2_parameters.update({'templates_dir': templates_dir})
            jinja2_parameters.update({'template_name': template_name})
            
            # Check if need to learn interfaces from yaml file
            interface_learn_name = jinja2_parameters.pop('interface_learn', False)
            if interface_learn_name:
                interface_list = []
                for interface in device.interfaces:
                    interface_list.append(interface)
                jinja2_parameters.update({interface_learn_name: interface_list})
                log.info('Interfaces found on device {}: {}'.format(
                    device.name, interface_list))
            device.api.configure_by_jinja2(**jinja2_parameters)
        except Exception as e:
            section.failed(
