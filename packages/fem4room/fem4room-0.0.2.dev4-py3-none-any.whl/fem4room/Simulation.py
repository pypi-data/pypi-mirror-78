...
class Simulation():
    ...
    def saveOutput(self, band, source_name, source_signal, receiver_signals, save_fs): 
        ...
        :param receiver_signals: Receivers signal
    ...
    def __ressample_pad(self,new_fs):
        ...
        for band in self.source_signals:
            new_size = int(np.max(self.time_arrays[band])*new_fs)
            rsource = np.zeros(int(max_duration*new_fs))
            _rsource = signal.resample(self.source_signals[band],new_size)
            rsource[:len(_rsource)] = _rsource
            rreceivers = []
            for rsig in self.receiver_signals[band]:
                rsig_new = np.zeros(int(max_duration*new_fs))
                rsig = signal.resample(rsig,new_size)
        ...
        return resampled_source_signals,resampled_receiver_signals
    ...
    def calcFrequencyResponse(self, receiverIndex = 0): #TODO EXPLAIN
        ...
        fs=16000
        resampled_source_signals,resampled_receiver_signals = self.__ressample_pad(fs)
        ...
        response = []
        for band in self.source_signals:
            _band_freqs = fft.rfftfreq(len(resampled_source_signals[band]),1/fs)
