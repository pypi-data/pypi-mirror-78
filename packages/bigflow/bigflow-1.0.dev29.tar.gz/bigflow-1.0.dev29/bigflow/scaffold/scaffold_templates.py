...
beam_pipeline_template = '''def dataflow_pipeline(gcp_project_id, staging_location, temp_location, region, machine_type, project_name):
    import uuid
...
class SimpleJob(object):
    def __init__(self, id, gcp_project_id, staging_location, temp_location, region, machine_type, bucket, project_name):
        self.id = id
        self.retry_count = 20
    ...
    def run(self, runtime):
        p = dataflow_pipeline(self.gcp_project_id, self.staging_location, self.temp_location, self.region, self.machine_type, self.project_name)
        count_words(p, WriteToText("gs://{}/beam_wordcount".format(bucket)))
...
simple_workflow = Workflow(
    ...
    definition=[SimpleJob(
        ...
        bucket=workflow_config['temp_location'],
...
from {project_name}.internationalports.workflow import scorer_workflow
