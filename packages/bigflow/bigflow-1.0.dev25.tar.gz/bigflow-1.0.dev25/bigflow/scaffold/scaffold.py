...
from bigflow.scaffold.scaffold_templates import beam_workflow_template, beam_processing_template, \
    ...
    basic_bq_config_template, advanced_bq_config_template
...
def format_templates(config):
    beam_config_template = basic_beam_config_template.format(project_id=config['projects_id'][0], bucket=config['composers_bucket'][0], project_name=config['project_name'])
    bq_config_template = basic_bq_config_template.format(project_id=config['projects_id'][0])
    test_templates = {'__init__.py': '', 'test_wordcount_workflow.py': test_wordcount_workflow_template, 'test_internationalports_workflow.py': test_internationalports_workflow_template}
    ...
    deployment_config_template = deployment_config_template.strip()
    deployment_config_template += '.resolve()'

    main_templates = {'project_setup.py': project_setup_template, 'deployment_config.py': deployment_config_template, 'Dockerfile': docker_template, 'README.md': readme_template.format(project_name=config['project_name'])}
...
def create_dirs_and_files(config, templates):
    ...
    workflows_path = Path(config['project_name'] + '/workflows')
