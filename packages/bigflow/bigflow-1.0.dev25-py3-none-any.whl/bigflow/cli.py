...
def project_type_input():
    return input("Would you like to create basic or advanced project?\n")


def project_number_input():
    return input('How many GCP projects would you like to use?\n')


def gcp_project_input():
    return input('GCP project\n')


def gcp_bucket_input():
    return input('GCP Bucket\n')


def environment_name_input():
    return input('Environment name\n')
...
def _cli_start_project():
    ...
    if project_type_input() != 'basic':
        for project in range(0, int(project_number_input())):
            config['projects_id'].append(gcp_project_input())
            config['composers_bucket'].append(gcp_bucket_input())
            config['envs'].append(environment_name_input())
    else:
