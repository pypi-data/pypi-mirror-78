...
class Transfer2SQLDB(object):
    ...
    def __init__(self, data_base_info=None):
        ...
        tmp_db_info = 'mysql+mysqlconnector://{0}:{1}@{2}:{3}/{4}?charset=utf8'.format(data_base_info["user"], data_base_info["password"], data_base_info["host"], str(data_base_info["port"]), data_base_info["db"])
    ...
    def get_tables(self) -> list:
        ...
        return list(x["Tables_in_{}".format(self.__db.db.encode("utf-8"))] for x in self.__cursor.fetchall())
    ...
    def __get_create_table_command(self, table_name, head_list, keys=None):
        ...
        tmp_head_list = list("_".join(key.lower().split() for key in head_list))
        tmp_col_type_str = ", ".join(list("{} {}".format(head, tmp_head_type_dict[head]) for head in tmp_head_list))
        ...
        if keys is not None:
            ...
            return "create table {0}({1}, {2}) DEFAULT CHARSET=UTF8MB4;".format(table_name, tmp_col_type_str, tmp_primary_key)
