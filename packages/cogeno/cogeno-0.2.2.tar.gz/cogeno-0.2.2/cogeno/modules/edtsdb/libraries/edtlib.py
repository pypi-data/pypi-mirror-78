...
class EDT:
    ...
    def __init__(self, dts, bindings_dirs, warn_file=None,
        ...
        self._init_binding_paths(bindings_dirs, bindings_exclude)
    ...
    def _init_binding_paths(self, bindings_dirs, bindings_exclude):
...
class Node:
    ...
    def _init_binding(self):
        ...
        elif self._node.name in ("chosen", "aliases", "memory", "reserved-memory"):
            # use default compatible for node
            self.compats = [self._node.name]
        ...
        elif self._node.parent and self._node.parent.name is "memory":
            # child of memory node
            self.compats = ["memory-region"]
        elif self._node.parent and self._node.parent.name is "reserved-memory":
            # child of reserved-memory node
            self.compats = ["reserved-memory-region"]
        ...
        # No binding found
        if ("default", None) in self.edt._compat2binding:
            # Use default binding if available
            self.matching_compat = "default"
            self._binding, self.binding_path = \
                self.edt._compat2binding["default", None]
        else:
...
def _dt_compats(dt):
    ...
    compats.extend(["aliases", "chosen", "default",
                    "fixed-partition", "gpio-led",
                    "memory", "memory-region",
                    "pinctrl-state", "pincfg",
                    "reserved-memory", "reserved-memory-region"])
