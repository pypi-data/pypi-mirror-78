...
in the directory specifed by --dbdir. Sections that do not exist in
...
'_order': ['build delete BUILDTAGS', 'build find MODULES', 'build list', 'build new MODULES', 'build remake BUILDTAG', 'build show BUILDTAG', 'build showmodules [BUILDTAG]', 'build showdependencies [BUILDTAG]', 'build showdependents [BUILDTAG]', 'build state BUILDTAG [NEW-STATE]', 'build try MODULES', 'build use MODULES'],
"": r"""
subcommands for maincommand "build"
+++++++++++++++++++++++++++++++++++
""",
...
be used to change the order of the builds shown.
""",
'build list': r"""
build list
::::::::::
...
The buildtag is optional, if omitted the command shows the modules for
...
of sumo. Here are examples for this:

Create a matching MODULES file for build 'AUTO-004' for an
application:

  sumo config make MODULES module -m "$(sumo build showmodules
  AUTO-004 --lines -b)"
...
++++++++++++
    ...
    have the statet 'stable' or 'testing'. If this option is given,
