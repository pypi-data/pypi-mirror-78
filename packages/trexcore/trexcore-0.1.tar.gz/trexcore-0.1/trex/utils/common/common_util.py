...
def normalized_mobile_phone(phone_no, default_country_code=lib_conf.DEFAULT_COUNTRY_CODE.upper()):
...
def format_mobile_phone(phone_no, default_country_code=lib_conf.DEFAULT_COUNTRY_CODE.upper()):
    parsed_mobile_phone = phonenumbers.parse(phone_no, region=default_country_code)
    return '+%s%s' % (parsed_mobile_phone.country_code, parsed_mobile_phone.national_number)
...
def get_offset_by_page_no(page_no, limit=lib_conf.PAGINATION_SIZE):
        ...
        if limit is not None:
            if isinstance(limit, string_types):
                try:
                    ...
                    limit = lib_conf.PAGINATION_SIZE
        else:
            limit = lib_conf.PAGINATION_SIZE
