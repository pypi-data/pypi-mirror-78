...
class TestHiveDb(TestCase):
    ...
    def test_hive_add_dbxd(self):
        ...
        _func = "usr"
        expected_calls = list()
        expected_calls.append(call(f"CREATE DATABASE `prv_tes_zi00_{_func}` "
                                   f"COMMENT `Données nécessaires aux cas d'usages métiers`"
                                   f" LOCATION '/data/prv_tes/zi00/{_func}'"))
        expected_calls.append(call(f"REVOKE ROLE `user_access_rw_prv_tes_zi00_{_func}`"))
        expected_calls.append(call(f"REVOKE ROLE `user_access_r_prv_tes_zi00_{_func}`"))
    ...
    def test_hive_add_db_exploited_zone_0(self):
        ...
        for _func in _functions:
            expected_calls.append(call(f"CREATE DATABASE `dev_zi00_{_func}` COMMENT `Dev "
                                       f"test base for automation`"
                                       f" LOCATION '/data/dev/zi00/{_func}'"))
            expected_calls.append(call(f"REVOKE ROLE `user_access_rw_dev_zi00_{_func}`"))
            expected_calls.append(call(f"REVOKE ROLE `user_access_r_dev_zi00_{_func}`"))
