...
class App(StateMixin, LoggingMixin, RollbarMixin):
    ...
    def load_state(self, state_key: [str, None] = None, auto_migrate: bool = False) -> dict:
        previous_state = super().load_state(state_key=state_key, auto_migrate=auto_migrate)
    ...
    def save_state(self, state_key: [str, None] = None, timestamp: [int, None] = None):
        if not timestamp:
            timestamp = self.state.get('last_processed_timestamp', 0)

        super().save_state(state_key=state_key, timestamp=timestamp)
