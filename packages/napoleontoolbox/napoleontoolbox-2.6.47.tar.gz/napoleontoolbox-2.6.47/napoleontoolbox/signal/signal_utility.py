...
def select_signals(results_df, metric, number, transaction_cost = False, number_per_year = 252):

    if 'close_return' not in results_df.columns and 'close' in results_df.columns :
        results_df['close_return'] = results_df['close'].pct_change()
    ...
    for sig in results_df.columns:
        if 'signal' in sig:
            ...
            sharpe_metric = None
            if metric == 'sharpe':
                sharpe_metric = metrics.sharpe(freqly_df['perf_return'].dropna(), period=number_per_year, from_ret=True)
            elif metric == 'calmar':
                sharpe_metric = metrics.calmar(freqly_df['reconstituted_perf'].dropna(), period=number_per_year)
            elif metric == 'drawdown':
                sharpe_metric = metrics.mdd(freqly_df['perf_return'].dropna())
            metrics_results[sig] = [sharpe_metric]
    ...
    return list(metrics_results_df.index)
