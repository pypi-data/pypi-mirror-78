...
def init(format='[%(asctime)s] [%(levelname)s] %(message)s', name='rbx'):
    LOGGING = {
        'version': 1,
...
class Logger:
    ...
    def __init__(self, format='[%(asctime)s] [%(levelname)s] %(message)s', logger=None, name='rbx'):
        if not logger:
            logger = logging
    ...
    def setup_logging(self):
        init(format=self.format, name=self.name)
...
class CloudLoggingLogger(Logger):
    ...
    Note that the name of the logger can be changed via the 'name' parameter.
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.cloud_logging_enabled:
            self.logger = self.LogWriter(name=self.name)
