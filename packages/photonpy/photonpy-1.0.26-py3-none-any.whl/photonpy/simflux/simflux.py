...
class SimfluxProcessor:
    ...
    def drift_correct(self, framesperbin=1, drift_fn=None, sigmaCoarseMultiplier=4,cuda=True):
        if drift_fn is None:
            from photonpy.smlm.drift_estimate import minEntropy

            names = ['SMLM', 'SIMFLUX']
            self.g_drift,self.sf_drift = [
                minEntropy(ds.get_xyI(), 
                  ...
                  sigmaCoarse=ds.get_crlb().mean(0)[0]*sigmaCoarseMultiplier,
                  dataname= names[i],
