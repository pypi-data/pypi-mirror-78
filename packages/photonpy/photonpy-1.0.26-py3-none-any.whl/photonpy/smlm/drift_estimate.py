...
#    roi_top = fit_sigma_2d(roi, initial_sigma=2)[[0, 1]]
    ...
    return (peak[1] + px - r + 1 - cc.shape[1] / 2) / 2, (peak[0] + py - r + 1 - cc.shape[0] / 2) / 2
...
def minEntropy(positions, framenum, crlb, framesperbin, imgshape, 
    ...
    with Context(debugMode=debugMode) as ctx:
        ...
        #xyI[:,:2] = xy
        #rcc(xyI, framenum, 5, np.max(imgshape), maxdrift)
        ...
        with tqdm.tqdm() as pbar:
            ...
            coarse_drift,score = method(
                xy, framenum, np.zeros((numframes,2)), sigmaPrecise, numIterations, step, maxdrift, 
        ...
        if display:
            ...
            ax[0].plot(drift_set1[:,0], '--', label=f'Drift X - set1, sigma={sigstr}')
            ax[0].plot(drift_set2[:,0], '--', label=f'Drift X - set2, sigma={sigstr}')
            ax[1].plot(drift_set1[:,1], '--', label=f'Drift Y - set1, sigma={sigstr}')
            ax[1].plot(drift_set2[:,1], '--', label=f'Drift Y - set2, sigma={sigstr}')
            ax[0].plot(drift[:,0], label=f'Drift X - full, sigma={sigstr}')
            ax[1].plot(drift[:,1], label=f'Drift Y - full, sigma={sigstr}')
            ax[0].plot(coarse_drift[:,0], label=f'Coarse Drift X - full, binsize={fpbCoarse}')
            ax[1].plot(coarse_drift[:,1], label=f'Coarse Drift Y - full, binsize={fpbCoarse}')
