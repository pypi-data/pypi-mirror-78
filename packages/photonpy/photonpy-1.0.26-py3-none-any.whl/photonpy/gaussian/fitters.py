...
from utils.peaks import get_peak_positions, get_rois
...
def estimate_sigma_from_movie(movie, roi_size):
    sigmas = []
    for i in range(len(movie)):
        positions = get_peak_positions(movie[i])
        rois = get_rois(movie[i], positions, roi_size=roi_size)

        for roi in rois:
            r = fit_sigma_2d(roi)
            sigmas.append(r[Params2D.SIGMA])
            if min(r[:2]) > roi_size / 2 - 3 and max(r[:2]) < roi_size / 2 + 3:
                sigmas.append(r[Params2D.SIGMA])

    print(sigmas)
    if len(sigmas)>0:
        psf_sigma = np.percentile(sigmas, 10)
        print(f"Sigma {psf_sigma:.3f} estimated from {len(sigmas)} spots")
    else:
        psf_sigma = None
