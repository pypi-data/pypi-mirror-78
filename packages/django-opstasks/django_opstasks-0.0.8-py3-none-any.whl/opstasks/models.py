...
from django_celery_beat import managers as beat_managers
...
from django_celery_results import managers as results_managers
...
class PeriodicTasks(models.Model):
    ...
    objects = beat_managers.ExtendedManager()
...
class PeriodicTask(models.Model):
    ...
    objects = beat_managers.PeriodicTaskManager()
    no_changes = False
...
class TaskResult(models.Model):
    ...
    task_id = models.CharField(
        ...
        help_text=u'Celery ID for the Task that was run')
    task_name = models.CharField(
        null=True, max_length=255, db_index=True, verbose_name=u'Task Name',
        help_text=u'Name of the Task which was run')
    task_args = models.TextField(
        null=True, verbose_name=u'Task Positional Arguments',
        help_text=u'JSON representation of the positional arguments used with the task')
    task_kwargs = models.TextField(
        null=True, verbose_name=u'Task Named Arguments',
        help_text=u'JSON representation of the named arguments used with the task')
    status = models.CharField(
        max_length=50, default=states.PENDING, db_index=True, choices=TASK_STATE_CHOICES,
        verbose_name=u'Task State', help_text=u'Current state of the task being run')
    worker = models.CharField(
        max_length=100, db_index=True, default=None, null=True,
        verbose_name=u'Worker', help_text=u'Worker that executes the task')
    content_type = models.CharField(
        max_length=128, verbose_name=u'Result Content Type',
        help_text=u'Content type of the result data')
    content_encoding = models.CharField(
        max_length=64, verbose_name=u'Result Encoding',
        help_text=u'The encoding used to save the task result data')
    result = models.TextField(
        null=True, default=None, editable=False, verbose_name=u'Result Data',
        help_text=(
            u'The data returned by the task. '
            u'Use content_encoding and content_type fields to read.'))
    date_created = models.DateTimeField(
        auto_now_add=True, db_index=True, verbose_name=u'Created DateTime',
        help_text=u'Datetime field when the task result was created in UTC')
    date_done = models.DateTimeField(
        auto_now=True, db_index=True,
        verbose_name=u'Completed DateTime',
        help_text=u'Datetime field when the task was completed in UTC')
    traceback = models.TextField(
        blank=True, null=True, verbose_name=u'Traceback',
        help_text=u'Text of the traceback if the task generated one')
    meta = models.TextField(
        null=True, default=None, editable=False, verbose_name=u'Task Meta Information',
        help_text=u'JSON meta information about the task, such as information on child tasks')

    objects = results_managers.TaskResultManager()
