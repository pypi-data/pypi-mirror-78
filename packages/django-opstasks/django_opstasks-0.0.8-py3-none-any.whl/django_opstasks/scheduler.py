...
import datetime
import django
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'opstasks.settings')
django.setup()
...
from django_opstasks.models import (
...
class ModelEntry(ScheduleEntry):
    ...
    def is_due(self):
        ...
        _timezone = self.app.timezone
        last_run_at_in_tz = maybe_make_aware(self.last_run_at).astimezone(_timezone)
    ...
    @classmethod
    def _unpack_options(
            cls, queue=None, exchange=None, routing_key=None,
            priority=None, headers=None, expire_seconds=None, **kwargs):
        del kwargs
        return {
            'queue': queue,
            'exchange': exchange,
...
class DatabaseScheduler(Scheduler):
    ...
    def all_as_schedule(self):
        LOGGER.debug('fetching database schedule')
        _sche = {}
        for model in self.Model.objects.enabled():
            try:
                _sche[model.name] = self.Entry(model, app=self.app)
            except ValueError:
                pass
        return _sche
    ...
    def schedule_changed(self):
        try:
            ...
            last, _ts = self._last_timestamp, self.Changes.last_change()
        except DatabaseError as exc:
            LOGGER.exception('database error: %r', exc)
            return False
        except InterfaceError:
            LOGGER.warning(
                'interface error in schedule_changed(), '
                'waiting to retry in next call...'
            )
            return False

        try:
            if _ts and _ts > (last if last else _ts):
                return True
        finally:
            self._last_timestamp = _ts
        return False
    ...
    def sync(self):
        ...
        try:
            ...
            LOGGER.exception('database error while scheduler sync: %r', exc)
        except InterfaceError:
            LOGGER.warning(
                'interface error while scheduler sync, '
                'waiting to retry in next call...'
            )
        finally:
