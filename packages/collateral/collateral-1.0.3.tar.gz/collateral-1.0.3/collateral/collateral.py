import functools
from collateral.exception import CollateralError
...
class BaseCollateral:
	...
	def collateral_map(self, f, notallnone=False):
		...
		try:
			...
			f = keep_errors(f)
			raise CollateralError(Collateral(*map(f, self.collaterals), notallnone=notallnone))
	...
	def __getattr__(self, attr):
		...
		try:
			...
			res = e.args[0]
			if all(isinstance(c, AttributeError) for c in res.collaterals):
	...
	def __dir__(self):
		...
		if self.collaterals:
			res = res.union(functools.reduce(set.intersection, map(set, map(dir, self.collaterals))))
		res = sorted(res)
		return res
	def _ipython_key_completions_(self):
		...
		keys = functools.reduce(set.intersection, keys)
		return keys
...
def pointwise_method(methname, callable=True):
	...
	if callable:
		def overloaded(self, *args, **kwargs):
			...
			return self.collateral_map(f, notallnone=True)
	else:
		def overloaded(self, *args, **kwargs):
...
def Collateral(*collaterals, notallnone=False):
	...
	ndir = functools.reduce(set.intersection, clsdirs) if lls else set()
	for methname in ndir:
		name = methname
		while name in BaseCollateral._notcollaterables or name in BaseCollateral.__dict__:
	...
	cls = type("Collateral", (BaseCollateral,), mdict)
