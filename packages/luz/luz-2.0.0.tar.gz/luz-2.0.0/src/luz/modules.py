...
class GraphNetwork(torch.nn.Module):
    def __init__(
        self, edge_model, node_model, global_model, num_layers: Optional[int] = 1
    ) -> None:
        super().__init__()
    ...
    def forward(
        ...
        for _ in range(self.num_layers):
            edges = self.update_edges(edges, edge_index, nodes, batch, u).reshape(
                edges.shape
            )

            aggregated_edges = self.aggregate_edges_local(
                nodes, edges, edge_index, batch, u
            )
            nodes = self.update_nodes(
                nodes, aggregated_edges, edge_index, batch, u
            ).reshape(nodes.shape)

            aggregated_node = self.aggregate_nodes(nodes, edge_index, batch)
            aggregated_edge = self.aggregate_edges(edges, edge_index, batch)
            u = self.update_global(aggregated_node, aggregated_edge, batch, u).reshape(
                u.shape
            )
