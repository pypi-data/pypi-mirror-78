...
class CloudvectorDAST(object):
    ...
    def _process_event_data(self, apis_to_check=[]):
        ...
        for event in events:
            ...
            if str(event["attributes"]["http_path"]) not in self.params_captured_in_traffic:
                self.params_captured_in_traffic[event["attributes"]["http_path"]] = {}
            if str(request["rsp_code"]) not in self.params_captured_in_traffic[event["attributes"]["http_path"]]:
                self.params_captured_in_traffic[event["attributes"]["http_path"]].update( { str(request["rsp_code"]) : {} } )
            if str(request["method"]) not in self.params_captured_in_traffic[event["attributes"]["http_path"]][request["rsp_code"]]:
                self.params_captured_in_traffic[event["attributes"]["http_path"]][request["rsp_code"]].update( { str(request["method"]) : [] } )
            params_found = {}
            for param, value in req_params_found.items():
            ...
            if params_found:
                ...
                self.params_captured_in_traffic[str(event["attributes"]["http_path"])][request["rsp_code"]][request["method"]].append(params_found)
                # if param in self.params_captured_in_traffic[request["rsp_code"]][str(event["attributes"]["http_path"])]:
                #     # self.params_captured_in_traffic[str(event["attributes"]["http_path"]).lower()] = {}
                #     #if value not in self.params_captured_in_traffic[request["rsp_code"]][str(event["attributes"]["http_path"])][
                #     #    param]:
