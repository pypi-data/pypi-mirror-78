...
class CloudvectorDAST(object):
                ...
                for _ in minfo.get("parameters", []):
                    ...
                    if _.get("value"):
                        if type(_.get("value")) is list:
                            #print("its a list....")
                            self.params_details_from_spec[str(_.get("name"))] = [x["name"] for x in _.get("value")]
                        # else:
                        #     print("not list...")
    ...
    def _get_spec_parsed(self, input_path):
        return self.openapispec_obj.parse_spec(self.apispec_two_path)
    ...
    def create_pyfixtures(self):
        ...
        nested_params={}
        more_params = []
        #print("???"+str(self.params_details_from_spec))
    ...
    def _create_assertions_map(self, url, params, req_payload, resp_payload):
        ...
        if url not in self.assertions_map:
            url = url.replace("//","/").replace("/", "_").replace("-", "_").replace("{", "").replace("}", "").split("?")[0]
    ...
    def create_pytest_methods(self, cv_events, custom_validations):
        ...
        for _ in cv_events:
            ...
            if event["url"]:
                ...
                #     0]
                api = str(event["http-req-url"]).lstrip("/").replace("/", "_").replace("-", "_").split("?")[0]
        ...
        for _ in cv_events:
            ...
            if _[1]:
                if _[1]["attributes"]["http_rsp_status_code"] not in ["200", "201"]:
                    continue
            if event["url"]:
                api = str(event["http-req-url"]).lstrip("/").replace("/", "_").replace("-", "_").split("?")[
                    0]
                if api not in apis_to_be_tested:
                    apis_to_be_tested[api] = {}
                #apis_to_be_tested[api]["method"] = event["method"]
