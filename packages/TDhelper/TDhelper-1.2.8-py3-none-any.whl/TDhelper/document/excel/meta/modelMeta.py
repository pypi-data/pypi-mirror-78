...
class modelMeta(type):
    def __new__(cls, name, bases, dct):
        attrs = {'mapping': {}, 'Meta': Meta, '__exit__': __exit__, 'readLine': readLine,
                 'close': close, '__initExcelHandle__': __initExcelHandle__, 'rows': rows, 'getCount': getCount, '_translateMode': _translateMode}
    ...
    try:
        if self.Meta.file:
            ...
            if m_extension == 'csv':
                ...
                m_offset= 1
                while True:
                    v= next(self.__sheetHandle__)
                    if v:
                        if m_offset<self.__record_offset:
                            m_offset+=1
                            continue
            ...
            if lineOffset <= self.__sheetHandle__.max_row:
                ...
                for (name, value) in self.mapping.items():
                    if value <= len(rowdata):
                        ...
                        raise Exception('mapping error:(%s,%s)' %
                                        (name, value))
