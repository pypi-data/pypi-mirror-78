...
class NelderMead:
    ...
    def init_subspaces(self, subspaces=None):
        ...
        if subspaces is None:
            ...
            if len(vpi) >= 3:
                ...
                # Last pair
                self.subspace_inds.append([0, vpi[self.n_pars_vary - 1]])
    ...
    def init_space(self, subspace_index=None):
        ...
        if subspace_index is not None:
            ...
            inds_vary = self.subspace_vary_inds[subspace_index]
        else:
            n = self.n_pars_vary
            inds = self.init_pars_vary_indices
            inds_vary = np.arange(n).astype(int)
        
        # Initialize the simplex
        right = np.zeros(shape=(n, n + 1), dtype=float)
        if subspace_index is not None:
            left = np.transpose(np.tile(self.xmin_numpy['value'][inds], (n + 1, 1)))
            diag = np.diag(self.init_pars_numpy['value'][inds])
            right[:, :-1] = diag
            self.current_simplex = left + right
        else:
            left = np.transpose(np.tile(self.init_pars_numpy['value'][inds], (n + 1, 1)))
            diag = np.diag(0.5 * self.init_pars_numpy['value'][inds])
            right[:, :-1] = diag
            self.current_simplex = left + right
    ...
    def solve_space(self, subspace_index=None):
        ...
        # Ensure the current simplex is updated
    ...
    def solve(self):
        ...
        for iteration in range(self.n_iterations):
            ...
            dx = self.compute_xtol(self.current_simplex)
            if dx > self.xtol:
                break
        ...
        if self.converged:
            try:
                xmin_err = self.compute_uncertainties(self.current_simplex)
            except:
