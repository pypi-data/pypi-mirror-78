...
class GridHandler():
    ...
    def submit(config_input:str, n_jobs=1, site=None, time=-1, mode='grid'):
        config = preprocess_config(config_input)
        if mode == 'grid':
            self.submit_grid(config, n_jobs=n_jobs, site=site, time=time)
        elif mode == 'idds':
            self.submit_idds(config, n_jobs=n_jobs, site=site)
        else:
    ...
    def submit_idds(config, n_jobs=1, site=None):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            command = self.format_idds_command(idds_config_path, 
                                               search_space_path,
                                               site=site)
    ...
    def submit_grid(config, n_jobs=1, site=None, time=-1):
        ...
        command = format_grid_command(config, site=site, time=time)
        with helper.cd(project_path):
            # submit jobs
            for _ in range(n_jobs):
                os.system(command)
    ...
    @staticmethod
    def format_idds_command(config_path, search_space_path, site=None):
        options = {'loadJson': config_path,
