...
class Ziva(SerialComm):
    def __init__(self):
        ...
        self.real_values = []
        self.info_params = {}
        # keeps info and real values together in same var
        self.data = {'real_values':self.real_values, 'info_params':self.info_params}
    ...
    def write_variable(self, name, value) -> dict:
        """Write variable.
        If variable is on of [rv_names, rv_units or rv_mask] we need to empty
        real values that were read before
        """
        if name in [RV_MASK, RV_UNITS, RV_NAMES]:
            self.real_values = []
    ...
    def read_info_variables(self):
        ...
        try:
            data['real_values'] = self.read_real_values()
        except Exception as e:
            data['real_values'] = None
