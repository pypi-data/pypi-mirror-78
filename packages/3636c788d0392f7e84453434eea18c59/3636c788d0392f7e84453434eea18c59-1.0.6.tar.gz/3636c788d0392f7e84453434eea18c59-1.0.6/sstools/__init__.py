...
# Airbnb Search API
def airbnb_search_api(place_id = "ChIJRYHfiwkB6DsRWIbipWBKa2k", city = "Alibag",state = "Maharashtra",min_price = 4000,items_per_grid = 50,section_offset = 1,items_offset = 50):

  url = f"https://www.airbnb.co.in/api/v3/ExploreSearch?locale=en-IN&operationName=ExploreSearch&currency=INR&variables=%7B%22request%22%3A%7B%22metadataOnly%22%3Afalse%2C%22version%22%3A%221.7.8%22%2C%22itemsPerGrid%22%3A{items_per_grid}%2C%22tabId%22%3A%22home_tab%22%2C%22refinementPaths%22%3A%5B%22%2Fhomes%22%5D%2C%22source%22%3A%22structured_search_input_header%22%2C%22searchType%22%3A%22pagination%22%2C%22mapToggle%22%3Afalse%2C%22roomTypes%22%3A%5B%22Entire%20home%2Fapt%22%5D%2C%22priceMin%22%3A{min_price}%2C%22federatedSearchSessionId%22%3A%22daf3b66f-051a-4dd1-8da0-22db10b36bc3%22%2C%22placeId%22%3A%22{place_id}%22%2C%22sectionOffset%22%3A{section_offset}%2C%22itemsOffset%22%3A{items_offset}%2C%22query%22%3A%22{city}%2C%20{state}%22%2C%22cdnCacheSafe%22%3Afalse%2C%22simpleSearchTreatment%22%3A%22simple_search_only%22%2C%22treatmentFlags%22%3A%5B%22monthly_stays_flexible_dates%22%2C%22stays_flexible_dates%22%2C%22simple_search_1_1%22%5D%2C%22screenSize%22%3A%22large%22%7D%7D&extensions=%7B%22persistedQuery%22%3A%7B%22version%22%3A1%2C%22sha256Hash%22%3A%22274161d4ce0dbf360c201612651d5d8f080d23820ce74da388aed7f9e3b00c7f%22%7D%7D"
...
# Airbnb Calendar API
def airbnb_calendar_api(listing_id,start_month=8,start_year=2020):
...
def airbnb_search(place_ids = ["ChIJRYHfiwkB6DsRWIbipWBKa2k"], max_iters = 2, min_price=4000):
  ...
  for place_id in place_ids:
    ...
    while counter<=max_iters:  
      ...
      response = airbnb_search_api(place_id = place_id, min_price = min_price , items_offset = offset)
      offset += 50
      ...
      if not response['data']['dora']['exploreV3']['sections']:
        ...
        all_items.extend(response['data']['dora']['exploreV3']['sections'][0]['items'])
  ...
  return_obj = items_df[['id','name','avgRating','amenityIds','bathrooms','bedrooms','city','lat','lng','personCapacity','publicAddress','canInstantBook','weeklyPriceFactor','monthlyPriceFactor','priceDropDisclaimer','priceString','rateType']]
  return_obj.drop_duplicates('id')

  return return_obj

# Airbnb Calendar DataFrame
def airbnb_calendar(listing_id,start_month=8,start_year=2020):
