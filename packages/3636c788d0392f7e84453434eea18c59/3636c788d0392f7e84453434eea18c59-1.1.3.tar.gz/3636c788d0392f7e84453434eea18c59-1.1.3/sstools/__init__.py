...
def vista_check_if_active(property_id="",slug=""):
  ...
  for i in [8,16,32,64,128]:
    ...
    elif type(price['error']) == 'str':

      if price['error'] == 'Booking Not Available for these dates':
        pass

      elif type(price['error'].split(" ")[4]) == "int":
        min_nights = price['error'].split(" ")[4]

    elif 'property_id' in price['error'].keys():
      return False
  return False

vista_check_if_active(956)
