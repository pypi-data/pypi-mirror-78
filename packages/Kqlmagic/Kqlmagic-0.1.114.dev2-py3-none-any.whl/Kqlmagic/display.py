        ...
        html_str = (
            ...
            function kql_MagicLaunchWindowFunction(file_path, window_params, window_name, host_or_text) {{
                ...
                if (host_or_text == 'text') {{
                    ...
                    url = file_path;
                }} else if (host_or_text.endsWith('.azureml.net') || host_or_text.endsWith('.azureml.ms')) {{
                    url = host_or_text + '/tree/' + file_path;
                }} else if (host_or_text == 'tree' || baseURI.search('.azureml.net') >= 0 || baseURI.search('.azureml.ms') >= 0) {{
                    let baseURI_parts = baseURI.split('/');
                    url = baseURI_parts[0] + '://' + baseURI_parts[2] + '/tree/' + file_path;
                }} else {{
                    var base_url = '';
