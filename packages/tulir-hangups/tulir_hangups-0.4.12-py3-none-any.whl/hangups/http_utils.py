...
async def check_error_response(resp: aiohttp.ClientResponse) -> None:
    if resp.status < 400:
        return
    try:
        error_data = (await resp.json())["error"]
        message = error_data.pop("message")
    except (aiohttp.ContentTypeError, json.JSONDecodeError, KeyError):
        # Not JSON or no error
        return
    raise exceptions.HangoutsError(message, **error_data)
...
class Session(object):
    ...
    async def fetch(self, method, url, params=None, headers=None, data=None):
        ...
        for retry_num in range(MAX_RETRIES):
            try:
                async with self.fetch_raw(method, url, params=params,
                    ...
                    async with async_timeout.timeout(REQUEST_TIMEOUT):
                        await check_error_response(res)
                        body = await res.read()
            ...
            except exceptions.HangoutsError as err:
                if err.status != 'UNAUTHENTICATED':
                    raise
                # These errors sometimes happen randomly even if we are authenticated
                last_err = err
                error_msg = "Server thinks we're not authenticated"
                # Give the server a chance to realize we're authenticated
            ...
            raise exceptions.NetworkError(error_msg) from last_err
