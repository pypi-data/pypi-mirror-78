        ...
        try:
            ...
            for logger in loggers:
                ...
                if callable(fn):
                    ...
                    appoptics_apm.util.logger.debug(
                        'update_filters: logger {}, {}(LogInjectionFilter).'.format(logger, f_name))
                else:
                    appoptics_apm.util.logger.warning(
                        'update_filters: could not call {} on logger {}.'.format(f_name, logger))
        except Exception as e:
            appoptics_apm.util.logger.error("AppOptics APM error: %s" % str(e))
        finally:
...
def wrap_logger_init(func, self, *f_args, **f_kwargs):
    ...
    try:
        if log_injector.is_injecting():
            ...
            appoptics_apm.util.logger.debug('wrap_logger_init adding LogInjectionFilter to logger {}.'.format(self))
    except Exception as e:
        appoptics_apm.util.logger.error(
...
def wrap_logging_module():
    if appoptics_apm.util.ready() and not log_injector.wrapped:
        try:
            ...
            appoptics_apm.util.logger.debug(
                'wrap_logging_module wraps {} of {} object'.format(fn.__name__, logger.__name__))
