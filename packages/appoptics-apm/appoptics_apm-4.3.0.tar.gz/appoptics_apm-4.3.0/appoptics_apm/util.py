...
import appoptics_apm.transaction_filter as transaction_filter
    ...
    def update_with_cnf_file(self, cnf_path):
        ...
        for section in cnf.sections():
            ...
            if len(subsections) == 3 and subsections[0] == 'transaction' and subsections[1] == 'filter':
                ...
                if not transaction_filter.add_transaction_filter_rule(config_dict):
                    logger.warning("Ignoring custom transaction setting [{}]".format(section))

            else:
                logger.warning('Invalid section: {s} found in {f}'.format(s=section, f=cnf_path))

        if transaction_filter.url_filter:
            logger.debug("Configured custom transaction filter {}".format(transaction_filter.url_filter))
...
def appoptics_apm_init():
    ...
    if sys_is_traceable():
        ...
        oboe_log_level = _OboeDebugLevel(appoptics_apm.config['debug_level'])
...
class Context(object):
    ...
    def set_transaction_name(self, trans_name=""):
        ...
        if not trans_name:
            logger.debug('Invalid transaction name:{n}'.format(n=_apm_str(trans_name)))
            return False
    ...
    def end_trace(self, event):  # Reports the last event in a trace
        ...
        if not http_span or do_metrics:
            ...
            if start_time is None:
                start_time = end_time
                logger.debug('trace start_time not recorded')
            else:
...
def sample_request(xtr, url_getter):
    ...
    """
    args = (xtr, transaction_filter.url_filter.filter_url_and_get_tracing(url_getter)
            ) if transaction_filter.url_filter and url_getter else (xtr, )
    (
        do_metrics,
        do_sample,
        sample_rate,
