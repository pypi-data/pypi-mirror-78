...
def get_exe(path, name):
    ...
    exe_blacklist = ["unins000.exe", "unitycrashhandler64.exe"]
    ...
    for directory in ["bin", "x64", "MossGame", "Binaries", "Win64"]:
...
header_count_dry, grid_count_dry, logo_count_dry = 0, 0, 0
...
for game in games:
    ...
    # Create the shortcut.
    if not shortcut and not args.dry_run:
    ...
    if args.artwork:
        ...
        header_url = f"https://steamcdn-a.akamaihd.net/steam/apps/{game['app_id']}/header.jpg"
        ...
        if shortcut.has_hero:
            if args.overwrite:
                if args.dry_run:
                    ...
                    shortcut.set_hero(url=header_url)
                    header_count += 1
...
if header_count_dry > 0 or grid_count_dry > 0 or logo_count_dry > 0:
