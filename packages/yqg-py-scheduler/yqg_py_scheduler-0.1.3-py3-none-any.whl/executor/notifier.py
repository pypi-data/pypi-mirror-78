...
from executor.executor import TriggerState
...
class Notifier(threading.Thread):
    ...
    def _do_notify(self, trigger):
        ...
        st = trigger.state()
        if st == TriggerState.INTERRUPTED:
            status = "INTERRUPTED"
        elif st == TriggerState.DONE:
            if trigger.exception():
                status = "FAILED"
            else:
                status = "SUCCESS"
        else:
            self._logger.error("unexpected trigger state %s. something must be wrong" % st)
            status = "FAILED"
        ...
        resp = requests.post("http://%s/internal/executeResult" % md.address(), json=data, timeout=5)
        data = resp.json()
        if data["status"]["code"] not in [0, 17003, 17004]:
            raise RuntimeError("Unexpected status code: %s" % data["status"]["code"])
