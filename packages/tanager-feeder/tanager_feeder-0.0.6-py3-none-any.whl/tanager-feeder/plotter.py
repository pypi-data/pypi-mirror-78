...
#         self.hues=[200,12,130,290,170,37,330]
        ...
        if self.x_axis!='theta': self.ax=fig.add_subplot(self.gs[0])
        else: 
        ...
        with plt.style.context(('default')):
            if self.x_axis!='theta': self.white_ax=self.white_fig.add_subplot(self.gs[0])
            else: self.white_ax=self.white_fig.add_subplot(self.gs[0],projection='polar')
...
#             if self.x_axis=='theta': self.plot_scale=self.plot_scale*2
    ...
    def set_title(self,title, draw=True):
        ...
        if self.x_axis!='theta': self.ax.title.set_position([0.5,1.02])
        else: self.ax.title.set_position([0.5,0.83])
        with plt.style.context('default'):
            self.white_ax.set_title(title, fontsize=24, color='black')
            if self.x_axis!='theta': self.white_ax.title.set_position([0.5,1.02])
            else: self.white_ax.title.set_position([0.5,0.83])
        if draw:
    ...
    def adjust_z(self, low, high):
        ...
        with plt.style.context(('default')):
            ...
            self.white_colorbar=self.fig.colorbar(self.white_contour, ax=self.white_ax, use_gridspec=False, anchor=(2,2))
                    ...
                    if (self.y_axis=='reflectance' or self.y_axis=='difference' or self.y_axis=='normalized reflectance') and self.x_axis=='wavelength':
                        ...
                        if self.exclude_artifacts: #If we are excluding data from the suspect region from 1050 to 1450 nm, divide each spectrum into 3 segments. One on either side of that bad region, and then one straight dashed line through the bad region. All the same color. Only attach a legend label to the first one so the legend only gets drawn once.
                            ...
                            if self.plotter.artifact_danger(g): #Only exclude data for high phase angle spectra
                                ...
                                self.lines.append(self.ax.plot(w_1,r_1, sample.linestyle, label=legend_label,color=color,linewidth=2))
                                if sample.linestyle!='--': self.lines.append(self.ax.plot(w_2,r_2,'--',color=color,linewidth=2))
                                else: self.lines.append(self.ax.plot(w_2,r_2,'.',color=color,linewidth=2))
                                ...
                                with plt.style.context('default'):
                                    ...
                                    self.white_lines.append(self.white_ax.plot(w_1,r_1, sample.linestyle, label=legend_label,color=white_color,linewidth=2))
                                    if sample.linestyle!='--': self.white_lines.append(self.white_ax.plot(w_2,r_2,'--',color=white_color,linewidth=2))
                                    else: self.white_lines.append(self.white_ax.plot(w_2,r_2,'.',color=color,linewidth=2))
                                    self.white_lines.append(self.white_ax.plot(w_3,r_3, sample.linestyle, color=white_color, linewidth=2))
                            else:
                                self.lines_drawn=True
                                self.lines.append(self.ax.plot(wavelengths,reflectance, sample.linestyle, label=legend_label,color=color,linewidth=2))
                                self.visible_data_headers.append(legend_label)
                                self.visible_data.append(reflectance)
                                
                                with plt.style.context('default'):
                                    self.white_lines.append(self.white_ax.plot(wavelengths,reflectance, sample.linestyle, label=legend_label,color=white_color,linewidth=2))
                            ...
                            if len(wavelengths)>50:
                                ...
                                self.lines.append(self.ax.plot(wavelengths,reflectance,'-'+sample.markerstyle, label=legend_label,color=color,linewidth=2,markersize=5))
                            ...
                            with plt.style.context('default'):
                                if len(wavelengths)>50:
                                    ...
                                    self.white_lines.append(self.white_ax.plot(wavelengths,reflectance, '-'+sample.markerstyle,label=legend_label,color=white_color,linewidth=2,markersize=5))
                    elif self.x_axis=='g':
                        ...
                        self.lines.append(self.ax.plot(sample.data[label][self.x_axis], sample.data[label][self.y_axis], sample.markerstyle,label=legend_label,color=color, markersize=6))
                        ...
                        if i==0 and j==0: #If this is the first line we are plotting, we'll need to create the polar axis.
                            c = self.ax.plot(theta, np.array(r),'-'+sample.markerstyle,color=color,label=legend_label, markersize=6)
                            
                            with plt.style.context('default'):
                                c_white = self.white_ax.plot(theta, np.array(r),'-'+sample.markerstyle,color=white_color,label=legend_label, markersize=6)
                            ...
                            c = self.ax.plot(theta, np.array(r),'-'+sample.markerstyle,color=color,label=legend_label,markersize=6)
                            with plt.style.context('default'):
                                c = self.white_ax.plot(theta, np.array(r),'-'+sample.markerstyle,color=white_color,label=legend_label,markersize=6)
...
#                                 self.white_ax.legend(bbox_to_anchor=(self.legend_anchor*1.2, 1), loc=1, borderaxespad=0.)
                        ...
                        self.visible_data.append(sample.data[label][self.y_axis])
                        self.markers_drawn=True
                        self.lines.append(self.ax.plot(sample.data[label][self.x_axis], sample.data[label][self.y_axis], '-'+sample.markerstyle,label=legend_label,color=color, markersize=5))
                        with plt.style.context('default'):
                            self.white_lines.append(self.white_ax.plot(sample.data[label][self.x_axis], sample.data[label][self.y_axis], '-'+sample.markerstyle,label=legend_label,color=white_color, markersize=5))
        ...
        self.white_ax.tick_params(axis='both',colors='black') #the plot style context should take care of this but doesn't.
    ...
    def draw_legend(self, legend_style):
        self.legend_style=legend_style #Does something if this method is called from Plot settings changing legend style
        if self.ax.get_legend()!=None: self.ax.get_legend().remove()
        if self.white_ax.get_legend()!=None: self.white_ax.get_legend().remove()
        ...
        if legend_style=='Full list':
            ...
            if self.x_axis!='theta':
                ...
                self.ax.legend(bbox_to_anchor=(self.legend_anchor*1.2, 0.85), loc=1, borderaxespad=0.)
                with plt.style.context(('default')): 
                    self.white_ax.legend(bbox_to_anchor=(self.legend_anchor*1.2, 0.85), loc=1, borderaxespad=0.)
        else:
...
#             self.white_fig.canvas.draw()  
            ...
            for sample in self.samples:
                ...
                if num_samples_plotted in [1,2]:
                    self.leg_ax.text(0, bottom, sample.name,verticalalignment='bottom', horizontalalignment='right',transform=self.leg_ax.transAxes,color='lightgray', fontsize=fontsize, rotation=90)
                    self.white_leg_ax.text(0, bottom, sample.name,verticalalignment='bottom', horizontalalignment='right',transform=self.leg_ax.transAxes,color='black', fontsize=fontsize, rotation=90)
                else:
                    self.leg_ax.text(0, bottom-.01, sample.name,verticalalignment='top', horizontalalignment='left',transform=self.leg_ax.transAxes,color='lightgray', fontsize=fontsize)
                    self.white_ax.text(0, bottom-.01, sample.name,verticalalignment='top', horizontalalignment='left',transform=self.leg_ax.transAxes,color='black', fontsize=fontsize)

                if sample not in self.legend_labels: continue
                num_colors=len(self.legend_labels[sample])                
                
                height=sample_height/num_colors
                for i in range(num_colors):
                    if i==0: 
                        self.leg_ax.text(left+width, bottom, self.legend_labels[sample][0].replace(sample.name,'').replace('(i','i').strip(')'),verticalalignment='bottom', horizontalalignment='left',transform=self.leg_ax.transAxes,color='lightgray', fontsize=fontsize)
                        self.white_leg_ax.text(left+width, bottom, self.legend_labels[sample][0].replace(sample.name,'').replace('(i','i').strip(')'),verticalalignment='bottom', horizontalalignment='left',transform=self.leg_ax.transAxes,color='black', fontsize=fontsize)
                    elif i==num_colors-1: 
                        self.leg_ax.text(left+width, bottom+height, self.legend_labels[sample][-1].replace(sample.name,'').replace('(i','i').strip(')'),verticalalignment='top', horizontalalignment='left',transform=self.leg_ax.transAxes,color='lightgray', fontsize=fontsize)
                        self.white_leg_ax.text(left+width, bottom+height, self.legend_labels[sample][-1].replace(sample.name,'').replace('(i','i').strip(')'),verticalalignment='top', horizontalalignment='left',transform=self.leg_ax.transAxes,color='black', fontsize=fontsize)

                    p = patches.Rectangle((left, bottom), width, height, facecolor=sample.colors[i],transform=self.leg_ax.transAxes)
                    self.leg_ax.add_patch(p)
                    p = patches.Rectangle((left, bottom), width, height, facecolor=sample.white_colors[i],transform=self.leg_ax.transAxes)
                    self.white_leg_ax.add_patch(p)
                    bottom+=height
                
                p=patches.Rectangle((left, bottom-sample_height), width, sample_height, fill=False, edgecolor='lightgray',transform=self.leg_ax.transAxes)
                self.leg_ax.add_patch(p)
                p=patches.Rectangle((left, bottom-sample_height), width, sample_height, fill=False, edgecolor='black',transform=self.leg_ax.transAxes)
                self.white_leg_ax.add_patch(p)
