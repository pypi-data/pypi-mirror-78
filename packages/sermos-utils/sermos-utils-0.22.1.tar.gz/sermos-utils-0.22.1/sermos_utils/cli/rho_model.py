""" Command Line Utilities for Sermos Rho Model APIs
"""
import json
import logging
import click
from sermos_utils.rho_model import search_rho_model
...
def model_search(access_key: str = None, model_name: str = None,
    ...
    """ Search for available models against Sermos Admin's public API.
        ...
        Arguments:
            ...
            model-name (optional): Model name to search for. Accepts wildcard.
                e.g. MyModelName, MyModel*
                If none provided, will search against all model names.
            version-pattern (optional): Version to search for. Accepts wildcard.
                e.g. 1.1.1, 1.*.*
                If none provided, will search against all versions.
    ...
    resp = search_rho_model(
        model_name, version_pattern, access_key, search_model_endpoint)
