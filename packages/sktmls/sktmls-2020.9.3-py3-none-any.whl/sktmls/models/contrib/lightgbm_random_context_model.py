...
class LightGBMRandomContextModel(MLSLightGBMModel):
    ...
    def __init__(
        ...
        - context_features: (optional) (list(str)) 컨텍스트 생성을 위한 피쳐 리스트
        - context_values: (optional) (list) 컨텍스트 값 리스트
    ...
    def predict(self, x: List[Any], **kwargs) -> Dict[str, Any]:
        ...
        y = self.models[0].predict(np.array([x[: len(x) - len(self.context_values)]])).flatten().tolist()[0]
        if y < self.cutoff:
            return {"items": []}

        contexts = [self.context_values[i] for i, x in enumerate(x[len(x) - len(self.context_values) :]) if x == "Y"]
        if len(contexts) == 0:
