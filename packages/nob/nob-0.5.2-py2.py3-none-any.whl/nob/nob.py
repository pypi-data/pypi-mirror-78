...
class _NobUtils(metaclass=ABCMeta):
    ...
    def np_serialize(self, minsize=0, compress=False):
        """Rewrite all numpy arrays of at least minsize to binary string
        ...
        """
        DO_NOT_SERIALIZE = (str, int, float, bool, dict, tuple, list)
        import numpy as np
        if compress:
            import zlib
        ...
        for path in self.paths:
            val = self[path][:]
            if not isinstance(val, DO_NOT_SERIALIZE) and val is not None:
                if val.size >= minsize:
    ...
    def np_deserialize(self):
        """Deserialize all numpy arrays from strings
