    ...
    def __repr__(self):
        return encode_utf8_if_py2(
            ('{cls}(username: {username}, role: {role}, '
                'can_share: {can_share}, user_id: {user_id})').format(
                    cls=self.__class__.__name__, username=self.username, role=self.role,
                    can_share=self.can_share, user_id=self.user_id
            )
        )
