...
class TrainingPredictions(APIObject):
    ...
    def _build_regression_dataframe(rows, is_datetime_partitioned, prediction_explanations):
        if is_datetime_partitioned:
            columns = ('row_id', 'partition_id', 'prediction', 'timestamp',)
        else:
        ...
        if prediction_explanations:
            ...
            columns += ('shap_remaining_total', 'shap_base_value',)
        ...
        for row in rows:
            ...
            if row.prediction_explanations:
                ...
                data_row += (
                    row.shap_metadata['shap_remaining_total'], row.shap_metadata['shap_base_value'],
                )
    ...
    def _get_classification_columns(
        ...
        if is_datetime_partitioned:
            columns += ('timestamp',)
        if prediction_explanations:
            columns += ('explained_class', 'shap_remaining_total', 'shap_base_value',)
            for i in range(len(prediction_explanations)):
                idx = i + 1
                columns += (_FEATURE.format(idx), _FEATURE_VAL.format(idx), _STRENGTH.format(idx),)
        return columns
