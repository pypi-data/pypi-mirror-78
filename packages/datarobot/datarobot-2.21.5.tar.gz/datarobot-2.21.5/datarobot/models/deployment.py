...
class Deployment(APIObject):
    ...
    @classmethod
    def create_from_custom_model_image(cls, custom_model_image_id, label, description=None,
                                       default_prediction_server_id=None,
        ...
        existing_model_packages = unpaginate(
            'modelPackages/', {'model_id': custom_model_image_id}, cls._client
        )

        try:
            model_package_id = next(existing_model_packages)['id']
        except StopIteration:
            # model package of the custom model image does not exists,
            # so create one
            model_package_payload = {'custom_model_image_id': custom_model_image_id}

            model_package_id = cls._client.post(
                'modelPackages/fromCustomModelImage/', data=model_package_payload
            ).json()['id']
    ...
    def get_target_drift(self, model_id=None, start_time=None, end_time=None):
        ...
        return TargetDrift.get(self.id, model_id=model_id, start_time=start_time, end_time=end_time)

    def get_feature_drift(self, model_id=None, start_time=None, end_time=None):
        ...
        return FeatureDrift.list(
            self.id, model_id=model_id, start_time=start_time, end_time=end_time
        )
...
class DeploymentListFilters(object):
    ...
    **Performing OR logic**
    ...
    def construct_query_args(self):
        ...
        if self.importance:
            query_args['importance'] = self._list_to_comma_separated_string(
                self.importance
            )
