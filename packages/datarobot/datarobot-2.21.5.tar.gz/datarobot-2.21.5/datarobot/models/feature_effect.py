...
class FeatureEffectMetadata(APIObject):
    ...
    def __repr__(self):
        return encode_utf8_if_py2(
            u"FeatureEffectMetadata({}/{})".format(self.status, self.sources)
        )
...
class FeatureEffectMetadataDatetime(APIObject):
    ...
    * ``sources`` : list(string).
    ...
    def __repr__(self):
        return encode_utf8_if_py2(
            u"FeatureEffectDatetimeMetadata({})".format(self.data)
        )
...
class FeatureEffects(APIObject):
    ...
    def __hash__(self):
        return hash((self.__class__.__name__, self.project_id, self.model_id, self.source,
                    self.backtest_index))
