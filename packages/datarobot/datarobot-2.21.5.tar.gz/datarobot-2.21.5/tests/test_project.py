...
def test_create_interaction_feature(project_endpoint, project, interaction_feature_server_data):
    ...
    assert request_body_to_json(responses.calls[0].request) == {
        'featureName': 'int0', 'features': ['cat1', 'cat2'], 'separator': '+'
    }
...
def test_get_relationships_configuration(project, relationships_configuration):
    ...
    for actual, expected in zip(result.dataset_definitions,
                                relationships_configuration['datasetDefinitions']):
    ...
    assert (
        actual["feature_derivation_window_end"] == expected["featureDerivationWindowEnd"]
    )
    assert (
        actual["feature_derivation_window_start"] == expected["featureDerivationWindowStart"]
    )
    assert (
