...
class TestExternalLiftChart(object):
    ...
    def create_response_data(lift_chart_bins_data, count):
        return [{'bins': lift_chart_bins_data, 'datasetId': 'dataset_id_{}'.format(i)} for i in
                range(count)]
    ...
    @responses.activate
    @patch('datarobot.models.external_dataset_scores_insights.external_lift_chart.'
    ...
    @patch('datarobot.models.external_dataset_scores_insights.external_lift_chart.'
           'DEFAULT_BATCH_SIZE', 10)
    def test_list_lift_chart_all_results_with_offset(self, lift_chart_bins_data, url,
                                                     project_id, model_id):
