...
from datarobot.models.feature_fit import FeatureFitMetadata, FeatureFitMetadataDatetime, FeatureFit
...
def feature_fit_server_data_datetime_holdout(feature_fit_server_data_datetime):
    feature_fit_server_data_datetime_holdout = dict(
        feature_fit_server_data_datetime
    )
...
@pytest.fixture
def feature_fit_response_holdout(
    feature_fit_server_data_holdout, feature_fit_url
):
    source = "holdout"
...
@pytest.fixture
def feature_fit_response_datetime(
    feature_fit_server_data_datetime, feature_fit_url_datetime
):
    source = "training"
    backtest_index = "0"
    responses.add(
        ...
        "{}?source={}&backtestIndex=".format(
            feature_fit_url_datetime, source, backtest_index
        ),
        status=200,
        content_type="application/json",
...
def feature_fit_url(project_id, model_id):
    return "https://host_name.com/projects/{}/models/{}/featureFit/".format(
        project_id, model_id
    )
...
@pytest.fixture
def feature_fit_completed_response(
    feature_fit_job_finished_server_data, job_url, feature_fit_url
):
    """
...
@pytest.fixture
def feature_fit_previously_ran_response_datetime(
    feature_fit_url_datetime, job_id
):
    """
    Loads a response that the given model has already ran its feature fit
     """
...
def test_get_feature_fit_metadata_datetime(
    ...
    for i, meta in enumerate(sorted(ff_metadata)):
        assert (
            meta.backtest_index ==
            feature_fit_metadata_datetime_sorted[i]["backtestIndex"]
        )
        assert meta.status == feature_fit_metadata_datetime_sorted[i]["status"]
        assert sorted(meta.sources) == sorted(
            feature_fit_metadata_datetime_sorted[i]["sources"]
        )


@responses.activate
@pytest.mark.usefixtures(
    "feature_fit_job_creation_response", "feature_fit_running_response"
)
...
def test_get_feature_fit_job_result_finished(feature_fit_server_data, one_model):
    ...
    assert len(feature_fit.feature_fit) == len(
        feature_fit_server_data["featureFit"]
    )
...
def test_get_feature_fit_job_result_finished_holdout(
    feature_fit_server_data_holdout, one_model
):
    feature_fit_job = one_model.request_feature_fit()
    ...
    assert feature_fit.project_id == feature_fit_server_data_holdout["projectId"]
    assert len(feature_fit.feature_fit) == len(
        feature_fit_server_data_holdout["featureFit"]
    )
...
def test_get_or_request_feature_fit_previously_requested(
    one_model, feature_fit_server_data
):
...
@responses.activate
@pytest.mark.usefixtures(
    "feature_fit_previously_ran_response", "feature_fit_running_response"
)
def test_get_or_request_feature_fit_currently_running_waits(
...
def test_wait_for_feature_fit_finished(one_model, feature_fit_server_data):
    ...
    feature_fit_job = one_model.request_feature_fit()
    feature_fit = feature_fit_job.get_result_when_complete(
...
def test_get_feature_fit_job_result_finished_datetime(
    ...
    assert feature_fit.project_id == feature_fit_server_data_datetime["projectId"]
    assert (
        feature_fit.backtest_index == feature_fit_server_data_datetime["backtestIndex"]
    )
    assert len(feature_fit.feature_fit) == len(
        feature_fit_server_data_datetime["featureFit"]
    )
...
def test_get_feature_fit_job_result_finished_holdout_datetime(
    ...
    feature_fit = feature_fit_job.get_result(params)
    assert (
        feature_fit.project_id == feature_fit_server_data_datetime_holdout["projectId"]
    )
    assert (
        feature_fit.model_id == feature_fit_server_data_datetime_holdout["modelId"]
    )
    assert (
        feature_fit.source == feature_fit_server_data_datetime_holdout["source"]
    )
    assert (
        feature_fit.project_id == feature_fit_server_data_datetime_holdout["projectId"]
    )
    assert (
        feature_fit.backtest_index ==
        feature_fit_server_data_datetime_holdout["backtestIndex"]
    )
    assert len(feature_fit.feature_fit) == len(
        feature_fit_server_data_datetime_holdout["featureFit"]
    )

    expected_ff = FeatureFit.from_server_data(
        feature_fit_server_data_datetime_holdout
    )
    assert expected_ff == feature_fit
...
def test_wait_for_feature_fit_never_finished_datetime(
...
def test_get_or_request_feature_fit_datetime(
    one_datetime_model, feature_fit_server_data_datetime
):
    backtest_index = "0"
    source = "training"
...
def test_get_feature_fit_invalid_source_datetime(one_datetime_model):
    ...
    source = "invalid"
    with pytest.raises(errors.ClientError):
        one_datetime_model.get_feature_fit(
            source=source, backtest_index=backtest_index
        )
