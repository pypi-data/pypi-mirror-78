...
from datarobot.models.visualai import ImageEmbedding, ImageActivationMap
...
def test_image_activationmpas(
    ...
    assert img0.image.id == visualai_activationmaps.get("activationMaps")[0].get(
        "imageId"
    )
    assert img0.overlay_image.id == visualai_activationmaps.get("activationMaps")[
        0
    ].get(
        "overlayImageId"
    )
    ...
    assert img0.model_id == model_id
    assert img0.feature_name == "image"
    assert img0.actual_target_value == visualai_activationmaps.get("activationMaps")[
        0
    ].get(
        "actualTargetValue"
    )
    assert img0.predicted_target_value == visualai_activationmaps.get("activationMaps")[
        0
    ].get(
        "predictedTargetValue"
    )
    assert img0.activation_values == visualai_activationmaps.get("activationMaps")[
        0
    ].get(
        "activationValues"
    )
    assert isinstance(img1, ImageActivationMap)
    assert img1.image.id == visualai_activationmaps.get("activationMaps")[1].get(
        "imageId"
    )
    assert img1.overlay_image.id == visualai_activationmaps.get("activationMaps")[
        1
    ].get(
        "imageId"
    )
    ...
    assert img1.model_id == model_id
    assert img1.feature_name == "image"
    assert img1.actual_target_value == visualai_activationmaps.get("activationMaps")[
        1
    ].get(
        "actualTargetValue"
    )
    assert img1.predicted_target_value == visualai_activationmaps.get("activationMaps")[
        1
    ].get(
        "predictedTargetValue"
    )
    assert img1.activation_values == visualai_activationmaps.get("activationMaps")[
        1
    ].get(
        "activationValues"
    )
