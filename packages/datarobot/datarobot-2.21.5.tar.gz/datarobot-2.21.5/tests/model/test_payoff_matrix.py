...
from tests.utils import request_body_to_json, assert_urls_equal
...
def mock_get_response(url, response, status=200, content_type="application/json"):
    if content_type == "application/json":
        ...
        body = response
    responses.add(
        responses.GET, url, status=status, content_type=content_type, body=body
    )
...
def assert_payoff_matrix(payoff_matrix, payoff_matrix_json):
    ...
    assert (
        payoff_matrix.false_positive_value == payoff_matrix_json["falsePositiveValue"]
    )
    assert (
        payoff_matrix.false_negative_value == payoff_matrix_json["falseNegativeValue"]
    )
...
def test_list(project_id, mocked_payoff_matrices, make_payoff_matrices_url):
    ...
    for payoff_matrix, payoff_matrix_json in zip(
        payoff_matrices, mocked_payoff_matrices["data"]
    ):
...
def test_create(one_model, project_url, payoff_matrix_id):
    ...
    assert (
        request_json.get("truePositiveValue") == submit_payoff_matrix["truePositiveValue"]
    )
