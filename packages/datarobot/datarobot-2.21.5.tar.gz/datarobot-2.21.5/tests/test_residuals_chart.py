...
def residuals_chart_validation_data(model_id, residuals_chart_example_data):
    return dict(
        source="validation", sourceModelId=model_id, **residuals_chart_example_data
    )
...
@pytest.fixture
def residuals_chart_parent_model_validation_data(
    parent_model_id, residuals_chart_example_data
):
    return dict(
        source="validation",
        sourceModelId=parent_model_id,
        **residuals_chart_example_data
    )


@pytest.fixture
def residuals_chart_parent_model_validation_api_response(
...
@pytest.fixture
def residuals_chart_parent_model_holdout_data(
    parent_model_id, residuals_chart_example_data
):
    return dict(
        source="holdout", sourceModelId=parent_model_id, **residuals_chart_example_data
    )
...
def test_instantiation(residuals_chart_validation_data):
    ...
    assert residuals_chart.coefficient_of_determination == residuals_chart_validation_data[
        "coefficientOfDetermination"
    ]
    assert (
        residuals_chart.residual_mean == residuals_chart_validation_data["residualMean"]
    )
    assert (
            residuals_chart.standard_deviation is None
    )
...
def test_future_proof(residuals_chart_validation_data):
    data_with_future_keys = dict(
        residuals_chart_validation_data, new_key="some future lift data"
    )
...
def test_get_frozen_validation_residuals_chart_with_fallback(
    ...
    model = Model(id=model_id, project_id=project_id, is_frozen=True)
    residuals_chart = model.get_residuals_chart(
        "validation", fallback_to_parent_insights=True
    )

    assert (
        residuals_chart.source == residuals_chart_parent_model_validation_data["source"]
    )
    ...
    assert residuals_chart.residual_mean == residuals_chart_parent_model_validation_data[
        "residualMean"
    ]
...
def test_get_frozen_all_residuals_charts_with_fallback(
    ...
    parent_list_response = {
        "residuals": residuals_chart_parent_model_validation_api_response[
            "residuals"
        ].copy()
    }
    ...
    assert residuals_chart.residual_mean == residuals_chart_parent_model_holdout_data[
        "residualMean"
    ]
