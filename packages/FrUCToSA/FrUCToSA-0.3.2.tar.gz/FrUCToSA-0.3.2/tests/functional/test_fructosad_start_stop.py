...
from fructosa.constants import (
    DEFAULT_OWN_FILE_LOGGING_PATH, FRUCTOSAD_DESCRIPTION, CONF_READ_MSG,
    FRUCTOSAD_DEFAULT_CONFIGFILE,
)


class BasicFructosadFunctionalityTest(BaseStartStop, unittest.TestCase):
    def setUp(self):
        self.wrapper_class = FrUCToSADWrapper
        self.program_starting_msg = FRUCTOSAD_STARTING_MESSAGE
        self.program_stop_msg = FRUCTOSAD_STOP_MESSAGE
        self.program_default_pidfile = FRUCTOSAD_DEFAULT_PIDFILE
        self.program_description = FRUCTOSAD_DESCRIPTION
        self.program_conf_read_msg = CONF_READ_MSG.format(
            config_file=FRUCTOSAD_DEFAULT_CONFIGFILE
        )
        self.program_default_configfile = FRUCTOSAD_DEFAULT_CONFIGFILE
        super().setUp()
...
@unittest.skip
class BasicFructosadFunctionalityInitScriptTest(SystemScriptStartStopTest):
    def test_fructosad_starts_and_stops(self):
        ...
        # that ``fructosad`` is not running before launching it:
        fructosad_process = self.find_fructosad()
        if fructosad_process:
            self.fail(
                "'fructosad' process found before starting! Stop it and come back to test."
            )
        #  Fine, now he can start testing this new promising tool!
        # For that, he launches the "fructosad" init script:
