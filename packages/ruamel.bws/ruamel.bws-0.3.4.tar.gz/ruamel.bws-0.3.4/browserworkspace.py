            ...
            while methods_with_sub_parsers:
                ...
                for method in all_methods_with_sub_parsers:
                    ...
                    if not arg or not isinstance(arg[0], string_types[0]):
                        arg = list(arg)
                        arg.insert(0, method.__name__)
                    sp = sub_parsers.add_parser(*arg, **method._sub_parser['kw'])
...
def version(version_string):
    return ProgramBase._pb_option('--version', action='version', version=version_string)
...
class AppConfig(object):
    ...
    def parse_args(self, *args, **kw):
        ...
        print('paropt', self._parser._optionals, len(opt._actions), len(opt._group_actions))
    ...
    def ewmh(self):
        ...
        for key in self._config._config:
            ...
            if val is None:
                print(
                    ...
                    '    defintions for key: {}'.format(self._config.get_file_name(), key)
                )
                continue
        ...
        if not start:
            print(
                ...
                'br-*/basenamestart defintions'.format(self._config.get_file_name())
            )
            sys.exit()
        self._browsers = {}
        self._nr_windows = 0
...
class bws_cmd(ProgramBase):
    ...
    def __init__(self):
        super(bws_cmd, self).__init__(formatter_class=SmartFormatter)
    ...
    @option('--unlock', action='store_true', help='create file specified by --unlock-file')
    def restore(self):
        bws = BrowserWorkspace(self._args, self._config)
