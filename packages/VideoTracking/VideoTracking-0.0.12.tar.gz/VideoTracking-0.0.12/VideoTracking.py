...
class VideoTrackingWindow(QMainWindow, Ui_VideoTrackingWindow):
    ...
    def play(self):
        ...
        self.timer.start(1000/self.FPS) #self.spinBox_FPS.value())        
        self.playButton.setIcon(
    ...
    #     self.prevPosition = position
        ...
        self.logging(f"Position set at time: {index*self.DeltaT/1000:.3f} s")
    ...
    def readVideo(self, path):
        ...
        for i, frame in enumerate(videogen):
            ...
            if progress.wasCanceled() or i > 200:
                break
    ...
    def processFrame(self, videoFrame):
        ...
        for shape in self.previous_shape:
            ...
            if this_tracker.isTracking(currentPos):
                ...
                new_box = this_tracker.run(self.rgbImage, currentPos)
    ...
    def stopTrackerClicked(self):
        ...
        if current_tracker.isTracking(self.frame_index):
            current_tracker.time_stop = self.frame_index
    ...
    def newShape(self):
        ...
        new_tracker = Tracker(time_start=self.frame_index, #self.mediaPlayer.position(),
                              time_stop=self.videolen,  # self.mediaPlayer.duration(),
...
class Tracker(QWidget):
    def __init__(self, time_start, time_stop, init_box, init_CVImage, FPS, parent=None):
        ...
        # self.frames = {f"{self.time_start}": init_CVImage}

        self.boxes = [self.box2rect(init_box)]
        # self.frames = [self.QImage2CVImage(init_CVImage)]
        self.frames = [init_CVImage]
        self.times = [self.time_start]

        h, w, ch = self.frames[-1].shape
        bytesPerLine = ch * w
        convertToQtFormat = QImage(self.frames[-1].data, w, h, bytesPerLine, QImage.Format_RGB888)
    ...
    def run(self, qimage, pos):
        ...
        img = qimage #self.QImage2CVImage(qimage)[:, :, :3]
        # ptr = qimage.bits()
    ...
    def getName(self):
        return f"From {self.time_start/1000}s to {self.time_stop} frames"
