...
from polyaxon.utils.formatting import flatten_keys, list_dicts_to_tabulate
...
class MultiRunPlot(RunClient):
    ...
    def get_runs_io(
        ...
        runs = [
            {"uuid": runs[r].uuid, "inputs": runs[r].inputs, "outputs": runs[r].outputs}
            for r in runs
        ]

        objects, _ = flatten_keys(
            objects=runs,
            columns=["inputs", "outputs"],
            columns_prefix={"inputs": "in", "outputs": "out"},
        )
        return list_dicts_to_tabulate(objects, humanize_values=False, upper_keys=False,)
