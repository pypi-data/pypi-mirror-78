...
def get_system_version(username,password):
...
def get_storage_information(username, password):
    auth_token = connect_user(username, password)
...
def create_user(username, password,name,email,
    ...
    url = f"{artifactory_url}/api/security/users/{name}"
    data = {'name': name,
            'email': email,
            'password': new_pass,
            'admin': is_admin,
...
def delete_user(username, password, user_to_delete):
    url = f"{artifactory_url}/api/security/users/{user_to_delete}"
    auth_token = connect_user(username, password)
