...
#    duration_hrs = tce['trapFit.duration_hrs']
    ...
    if clip['config.detrendType'] != "tess_2min" and (clip['config.detrendType'] != "eleanor_old") and (clip['config.detrendType'] != "tess_FFI_eleanor"):
...
#        titleStr = "EPIC: %i  %s" %(epic, titleStr)
    ...
    else:#elif clip['config.detrendType'] == "tess_2min":
...
#    itr_mean_img_, oot_mean_img_, diff_mean_img_ = np.asarray(itr_mean_img_), np.asarray(oot_mean_img_), np.asarray(diff_mean_img_)
    ...
    isnan = np.isnan(time)#clip['detrend.detrendFlux'])
    time = time[~isnan]
    cube = cube[~isnan]
...
#    xxxx
    ...
    disp = lambda x: mp.imshow(x, cmap = parula_map, origin = "bottom", extent = extent_)#interpolation = "nearest", extent = extent_)


    kwargs = {'origin':'lower', 'interpolation':'none','cmap':parula_map, 'extent':[0, ss_[1],0, ss_[0]]}
    mp.subplot(221)

#    disp(diff_mean_img_)    
    im = mp.imshow(diff_mean_img_, **kwargs)
#
#
    ...
    Z = g(X[:-1],Y[:-1])
    mp.contour(Z[::-1], [0.5], colors = 'w', linestyles = 'dashed', alpha = 1.0, linewidths=[2], \
            extent=[0-0., x[:-1].max()-0.,0-0., y[:-1].max()-0.], origin = 'upper')
#
#
#    mp.plot(np.mean(diffCol_prf), np.mean(diffRow_prf), 'r*', ms = 12, mec = 'k', label="DIFF")
    mp.plot(diffCol_prf, diffRow_prf, 'ro')#, label="Indiv Diff")

#    if clip['config.detrendType'] == "tess_2min":
#        ra_, dec_ = hdr_['RA_OBJ'], hdr_['DEC_OBJ']
#        coord = SkyCoord(hdr_['RA_OBJ'], hdr_['DEC_OBJ'], unit = "deg")
#    elif clip['config.detrendType'] == "tess_FFI_eleanor":
#        ra_, dec_ = hdr_['CEN_RA'], hdr_['CEN_DEC']
#        coord = SkyCoord(hdr_['CEN_RA'], hdr_['CEN_DEC'], unit = "deg")
...
#    print(TIC_x,TIC_y)
    ...
    mp.plot(TIC_x, TIC_y, 'k*', mfc = "None", ms = 22, mew = 1.5, label = 'TIC')

    mp.plot(np.nanmean(diffCol_prf),np.nanmean(diffRow_prf),'ro',mfc = "None",mec='r',mew = 1.5, ms=18, label='Avg DIFF')
...
#    disp(oot_mean_img_)
    im = mp.imshow(oot_mean_img_, **kwargs)
    mp.contour(Z[::-1], [0.5], colors = 'w', linestyles = 'dashed', alpha = 1.0, linewidths=[2], \
            extent=[0-0., x[:-1].max()-0.,0-0., y[:-1].max()-0.], origin = 'upper')
    mp.plot(np.nanmean(ootCol_prf), np.nanmean(ootRow_prf), 'mv', mfc = "None", ms = 10, mew = 2., label="OOT")
    mp.plot(TIC_x, TIC_y, 'k*', mfc = "None", ms = 22, mew = 1.5, label = 'Target')
...
#    disp(itr_mean_img_)
    im = mp.imshow(itr_mean_img_, **kwargs)
    mp.contour(Z[::-1], [0.5], colors = 'w', linestyles = 'dashed', alpha = 1.0, linewidths=[2], \
            extent=[0-0., x[:-1].max()-0.,0-0., y[:-1].max()-0.], origin = 'upper')
#    mp.plot(ootCol_prf, ootRow_prf, 'mv', ms = 10, label="OOT")

    mp.plot(TIC_x, TIC_y, 'k*', mfc = "None", ms = 22, mew = 1.5, label = 'Target')

    mp.plot(np.nanmean(diffCol_prf),np.nanmean(diffRow_prf),'ro',mfc = "None",mec='r',mew = 1.5, ms=18, label='Avg DIFF')
    mp.plot(np.nanmean(ootCol_prf), np.nanmean(ootRow_prf), 'mv', mfc = "None", ms = 10, mew = 1.5, label="OOT")
...
#    ootCol_prf, ootRow_prf = ootCol_psf, ootRow_psf
    ...
    time = clip['serve.time']
    if clip['config.detrendType'] == "tess_2min":
        flux_ = clip['serve.detrendFlux']
    elif clip['config.detrendType'] == "tess_FFI_eleanor":
        flux_ = clip['detrend.flux_frac']
    ...
    for jj in range(len(transits)):
        ...
        error_code_ = np.hstack((error_code_, out[key]['errorCode']))

        no_data_ = np.nanmean(flux_[cin[0]:cin[1]])
         
        if (out[key]['errorCode'] > 7) or (np.isnan(no_data_) == True):
            transit_flags_[jj] = True
        else:        
            plot = False
            before, after, diff = generateDiffImg(cube, cin, plot=plot)
    ...
    transits = transits[~transit_flags_]
    error_code_ = error_code_[~transit_flags_]

    diff_img_, oot_img_ = diff_img_[~transit_flags_], oot_img_[~transit_flags_]
    ootCol_prf, ootRow_prf = ootCol_prf[~transit_flags_], ootRow_prf[~transit_flags_]
    diffCol_prf, diffRow_prf = diffCol_prf[~transit_flags_], diffRow_prf[~transit_flags_]

    oot_mean_img_ = np.nanmedian(oot_img_, axis = 0)
    diff_mean_img_ = np.nanmedian(diff_img_, axis = 0)
#    itr_mean_img_ = oot_mean_img_
...
#	subplot_ = 330 + int(ii+1)#10*(int(float(ii)//3.) + 1) + int(ii+1)
    ...
    for ii in range(transit_number_):
        ...
        if plot_ == 'plot_flux_':
            ...
            kwargs = {'origin':'lower', 'interpolation':'none','cmap':parula_map, 'extent':[0, ss_[1],0, ss_[0]]}
#            disp = lambda x: mp.imshow(x, **kwargs)#cmap = parula_map, origin = "bottom", interpolation = "nearest", extent = extent_)
#            disp(diff_img_[ii,:,:])
            im = mp.imshow(diff_img_[ii,:,:], **kwargs)
#
#
#
            if clip.config.detrendType == "tess_2min":
...
#
            ...
            TIC_x,TIC_y=starloc[0,0]+0.5, starloc[0,1]+0.5
#    if clip['config.detrendType'] == "tess_2min":
#        TIC_x, TIC_y = clip['serve.tpfHeader']['1CRPX4']-0.5, clip['serve.tpfHeader']['2CRPX4']-0.5
#    elif clip['config.detrendType'] == "tess_FFI_eleanor":
#        tmp_tpf = hdr_['TDIM4']
#        tmp_tpf = eval('(' + tmp_tpf + ')')
#        TIC_x,TIC_y=hdr_['1CRPX4']-int((tmp_tpf[0]-hdr_['TPF_H'])/2)-0., hdr_['2CRPX4']-int((tmp_tpf[1]-hdr_['TPF_W'])/2)-0.

            mp.plot(TIC_x, TIC_y, 'k*', mfc = "None", ms = 22, mew = 1.5, label = 'Target')

            mp.plot(diffCol_prf[ii], diffRow_prf[ii], 'ro', mfc = 'None', mew = 2, ms = 10)
#            mp.plot(ootCol_prf[ii], ootRow_prf[ii], 'w*', ms = 16)
            mp.colorbar(im, fraction=0.046, pad=0.04)
            
#            print(int(error_code_[ii]), np.nanmedian(diff_img_[ii,:,:]))

            if int(error_code_[ii]) == 0:
                title_ = 'Fit OK'
            elif int(error_code_[ii]) == 1:
                title_ = 'Before fail'
            elif int(error_code_[ii]) == 2:
                title_ = 'Diff fail'
            elif int(error_code_[ii]) == 4:
                title_ = 'After fail'
            elif int(error_code_[ii]) == 3:
                title_ = 'Before & Diff fail'
            elif int(error_code_[ii]) == 5:
                title_ = 'Before & After fail'
            elif int(error_code_[ii]) == 6:
                title_ = 'Diff & After fail'       
            elif (int(error_code_[ii]) == 7):# or (int(error_code_[ii]) == 3) or (int(error_code_[ii]) == 5):
                title_ = 'All fail'
            mp.title(title_)# + str(np.round(np.nanmedian(diff_img_[ii,:,:]),2)))
#            mp.title('Error code ='+str(int(error_code_[ii])))
#		mp.tight_layout()
    return
