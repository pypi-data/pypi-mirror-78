...
# Identify outliers
    ...
    n_points_ = 71# if clip['bls.period'] < XXX else 21
...
def blsTask(clip):
    ...
    # Trying to fix an issue where BLS locks onto very deep single-point artifacts
    # by clipping points 5-sigma too small wrt median
    time_for_bls, flux_for_bls = time_days[idx], flux_norm[idx]   
    sigma = np.sqrt(np.mean(flux_for_bls**2))
    too_low_ = np.abs(flux_for_bls) < 5*sigma
    time_for_bls, flux_for_bls = time_for_bls[too_low_], flux_for_bls[too_low_]
    # Trying to fix an issue where BLS locks onto very deep single-point artifacts
    #

    period, epoch, duration, depth, bls_search_periods, convolved_bls = \
        bls.doSearch(time_for_bls, flux_for_bls, minPeriod, maxPeriod)
