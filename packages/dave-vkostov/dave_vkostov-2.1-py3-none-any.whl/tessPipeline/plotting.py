...
from dave.tessPipeline.sweet import SweetFitOotFlux
...
def plotData(clip, nPanel=3):
    ...
    flux = clip['serve.detrendFlux']#clip['detrend.flux_frac']
...
#    except KeyError, e:
    ...
    if (clip['config.detrendType'] != "tess_2min") and (clip['config.detrendType'] != "tess_FFI_eleanor"):# and (clip['config.detrendType'] != "eleanor") :
...
def summaryPlot1(output):
    ...
    if (output.config.detrendType != "tess_2min") and (output.config.detrendType != "eleanor_old") and (output.config.detrendType != "tess_FFI_eleanor"):
...
def undetrendedFolded(clip):
    """
    Plot the undetrended folded light curve
    """
    ...
    time, flux, model = time[~flags], flux[~flags], model[~flags]

    plt.subplot(2,2,(1,2))
#   Plot undetrended folded data
#    phi = np.fmod(time - epoch_btjd + 0.25*period_days, period_days)
    phi = (time - epoch_btjd + 0.25*period_days)/period_days % 1
    plt.plot(phi, 1e6*flux, 'ko', ms=4)
#    plt.plot(period_days + phi, 1e6*flux, 'o', color='#888888', ms=4, mec="none")
    plt.xlim(0., 1)#*period_days)
    plt.title('Porb = ' + str(np.round(period_days,2)) + ' days', fontsize = 18)

    idx = markTransitCadences(time, period_days, epoch_btjd, dur_days)
#    for per in [period_days]:#, period_days, 2* period_days]:
#        phase = np.fmod(time - epoch_btjd + 0.*per, per)
#        fObj = lsf.Lsf(phase[~idx], flux[~idx], None, 2, lsf.sine, period=np.max(phase))
#    plt.plot(phase[~idx], flux[~idx], 'k.')
#    plt.plot(phase[~idx], fObj.getBestFitModel(), 'r.')

# Plot Lomb-Scargle
    frequency, power = LombScargle(time[~idx], 1e6*flux[~idx]).autopower(\
    minimum_frequency=1./2./period_days, maximum_frequency = 1./0.6/period_days, samples_per_peak=50)
    ...
    plt.plot((time[~idx] - epoch_btjd + 0.25*(1/best_frequency))/(1/best_frequency) % 1, 1e6*flux[~idx], 'k.')
    plt.title('P(Lomb-Scargle) = ' + str(np.round(1/best_frequency,2)) + ' days', fontsize = 18)
    plt.xlim(0.,1)

#   Plot trapezoid model
#    x = phi[~flags]
#    y = 1e6*model[~flags]
#    idx = np.argsort(x)
#    plt.plot(x[idx], y[idx], 'r-')
#    plt.plot(period_days + x[idx], y[idx], 'r-')
