...
class TaggerModel(tf.keras.Model):
    ...
    def train_step(self, data):
        ...
        if hasattr(self, 'optimizer_encoder'):
            if hasattr(self.optimizer_encoder,
                       '_name') and self.optimizer_encoder._name == 'AdamW':
                self.optimizer_encoder.apply_gradients(
                    zip(gradients[:encoder_layers],
                        self.trainable_variables[:encoder_layers]),
                    decay_var_list=[
                        v.ref()
                        for v in self.trainable_variables[:encoder_layers]
                        if ('layernorm' not in v.name.lower()
                            and 'layer_norm' not in v.name.lower()
                            and 'bias' not in v.name.lower())
                    ])
            ...
            if hasattr(self.optimizer_encoder,
                ...
                self.optimizer.apply_gradients(
                    ...
                    decay_var_list=[
                        v.ref() for v in self.trainable_variables
                        if ('layernorm' not in v.name.lower()
                            and 'layer_norm' not in v.name.lower()
                            and 'bias' not in v.name.lower())
                    ])
