...
class ImplCompute(ImplBase):
    ...
    def view_status(self, run_name, tensorboard, mirror, target, workspace, job, monitor, escape_secs, auto_start, 
        ...
        if not target:
            ss_info = self.config.get_service_set_info()
            ...
            with KeyPressChecker() as checker:
                ...
                while simple_status != "completed":
                    ...
                    if ch in ["+", "-"] and node_count > 1:
                        ...
                        # set new context
                        service_node_info = service_info_by_node[node_index]
    ...
    @option("workspace", default="$general.workspace", help="the workspace that contains the job to be cancelled")
    ...
    def cancel_job(self, job_id, workspace):
