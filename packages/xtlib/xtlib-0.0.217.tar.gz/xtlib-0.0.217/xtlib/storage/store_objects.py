...
class StoreBlobObjs():
    ...
    def validate_storage_and_mongo(self, mongo):
        ...
        if storage_info["storage_format"] != constants.STORAGE_FORMAT:
            errors.store_error("XT storage format {} expected but found: {}".format(constants.STORAGE_FORMAT, storage_info["storage_format"]))

        if not paired_storage:
            mongo_info = {"paired_storage": storage_name, "storage_format": constants.STORAGE_FORMAT}
            mongo.set_mongo_info(mongo_info)

        if not paired_mongo:
            storage_info = {"paired_mongo": connected_mongo, "storage_format": constants.STORAGE_FORMAT}
            self._set_storage_info(storage_info)

        if mongo_info["storage_format"] != constants.STORAGE_FORMAT:
            errors.store_error("XT mongo format {} expected but found: {}".format(constants.STORAGE_FORMAT, mongo_info["storage_format"]))
    ...
    def _create_info_container_if_needed(self):
        ...
        if not container_exists or not self.provider.does_blob_exist(info_container, constants.STORAGE_INFO_FILE):
            ...
            info = {"storage_format": constants.STORAGE_FORMAT, "paired_mongo": None}
            text = json.dumps(info)
...
class RunBlobs(BlobsObject):
    def __init__(self, store, ws_name, run_name):
        ...
        parent_num = run_helper.get_parent_run_number(run_name)
        self.base_path = "jobs/job{}/runs/{}".format(parent_num, run_name)
