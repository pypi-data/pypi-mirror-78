...
class ImplStorage(ImplBase):
    ...
    #---- VIEW SERVICE SET command ----
    @argument(name="name", default="$service-set", required=False, help="the name of the service set to use")
    @example("xt view service-set dilbert", task="show information about the specified service set")
    @command(kwgroup="view", help="display information about the active or specified service set")
    def view_service_set(self, name):
        # for now, its just an echo of your CURRENT (or specified) team name
        ss_info = self.config.get_service_set_info(name)

        ss_info = dict(ss_info)    # make copy we can change
        name = ss_info["name"]
        del ss_info["name"]

        console.print("service set {}: {}".format(name, ss_info))

    #---- LIST SERVICE SETS command ----
    @example("xt list service sets", task="show the service sets defined in the xt config file")
    @command(kwgroup="list", help="show the service sets defined in the xt config file")
    def list_service_sets(self):
        # for now, its just an echo of your CURRENT (or specified) team name
        ss_infos = self.config.get("service-sets")

        console.print("service sets")
        for name, ss_info in ss_infos.items():
            console.print("  {}: {}".format(name, ss_info))
    ...
    def start_storage_tracker(self):
        ...
        ss_info = self.config.get_service_set_info(None)
