...
def get_command_dicts(prev_exper, curr_exper, browse_flag, browse_opt, timeout_opt, templ, archives_dir, 
	...
	return [
	    ...
	    {"title": "list runs with the 'good_run' tag", "xt_cmd": "xt list runs --tags-any=good_run"},
	    ...
	    {"title": "let's see which runs are now tagged with 'top5", "xt_cmd": "xt list runs --tags-any=top5"},
	    ...
	    {"title": "download all source code, output, and logs for run4", "xt_cmd": "xt extract run4 {} {}".format(archives_dir, browse_opt)},
...

	    {
            ...
            "xt_cmd": "xt list runs --job={} --sort=metrics.test-acc".format(big_job)
        },
	    ...
	    #     "xt_cmd": "xt explore {} {}".format(big_job, timeout_opt),
        #     "needs_gui": True
	    # },
        ...
        # SINGLE PLOT of 10 RUNS
        {
	        ...
	        "xt_cmd": '!xt plot {} test-acc {}'.format(big_job, timeout_opt),
			"needs_gui": True
	    },
        ...
        # 2 METRICS, 2x5 MATRIX (break on run)
        {
	        ...
	        "xt_cmd": '!xt plot {} train-acc, test-acc --break=run --layout=2x5 {}'.format(big_job, timeout_opt),
	        "needs_gui": True
	    },
        ...
        # 2 METRICS, 2x1 MATRIX (break on col)
        {
	        ...
	        "xt_cmd": '!xt plot {} train-acc, test-acc --break=col --layout=2x1 {}'.format(big_job, timeout_opt),
	        "needs_gui": True
	    }
