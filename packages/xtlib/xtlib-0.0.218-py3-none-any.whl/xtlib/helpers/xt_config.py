...
class XTConfig():
    ...
    def create_vault_if_needed(self):
        if not self.vault:
            ...
            service_set_name = self.get("service-set")

            self.vault = xt_vault.XTVault(vault_url, service_set_name)
    ...
    def get_storage_creds(self):
        ...
        ss_info = self.get_service_set_info()
        storage_name = utils.safe_value(ss_info, "storage")
        if not storage_name:
            errors.config_error("'storage' property must be set in service-set '{}'".format(ss_info["name"]))
    ...
    def get_mongo_creds(self):
        ...
        if not mongo_name:
            errors.config_error("'mongo' property must be set in service-set '{}'".format(ss_info["name"]))
    ...
    def get_service_set_info(self, service_set_name=None):
        if not service_set_name:
            service_set_name = self.get("service-set", suppress_warning=True)
            
        if not service_set_name:
            errors.config_error("XT config file is missing required property: service-set")
        
        service_set_info = self.get("service-sets", service_set_name, suppress_warning=True)
        if not service_set_info:
            errors.config_error("XT config file is missing definition for service-set: {}".format(service_set_name))

        service_set_info["name"] = service_set_name
        return service_set_info
...
def _apply_basic_mode_rules(config):
    ...
    if not config.get("general", "advanced-mode"):
        ...
        ss_info = config.get_service_set_info()
