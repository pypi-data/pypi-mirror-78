...
from competitions import ApiClient as CompetitionsApiClient
from competitions import Configuration as CompetitionsConfiguration
from extended.models.kaggle_models_extended import Competition
from extended.models.kaggle_models_extended import Dataset
from extended.models.kaggle_models_extended import DatasetNewResponse
from extended.models.kaggle_models_extended import DatasetNewVersionResponse
from extended.models.kaggle_models_extended import File
from extended.models.kaggle_models_extended import FileUploadInfo
from extended.models.kaggle_models_extended import Kernel
from extended.models.kaggle_models_extended import KernelPushResponse
from extended.models.kaggle_models_extended import LeaderboardEntry
from extended.models.kaggle_models_extended import ListFilesResult
from extended.models.kaggle_models_extended import Metadata
from extended.models.kaggle_models_extended import Submission
from extended.models.kaggle_models_extended import SubmitResult
from kaggle import ApiClient as KaggleApiClient
from kaggle import Configuration as KaggleConfiguration
from kaggle import Collaborator
from kaggle import DatasetColumn
from kaggle import DatasetNewRequest
from kaggle import DatasetNewVersionRequest
from kaggle import DatasetUpdateSettingsRequest
from kaggle import DatasetUploadFile
from kaggle import KaggleServiceApi
from kaggle import KernelPushRequest
from kaggle import License
...
class KaggleApi:
    ...
    # Set as None so that help commands without a key don't fail
    competitions = None
    ...
    def authenticate(self):
        ...
        if self.CONFIG_NAME_USER not in config_data \
            ...
            if os.path.exists(self.config):
                ...
                print('Could not find {}. Make sure it\'s located in'
    ...
    def _load_config(self, config_data):
        ...
        kaggle_configuration = KaggleConfiguration()
        competitions_configuration = CompetitionsConfiguration()

        # Workout for extra '/' being inserted into routes
        if kaggle_configuration.host[-1] == '/':
            kaggle_configuration.host = kaggle_configuration.host[:-1]
        if competitions_configuration.host[-1] == '/':
            competitions_configuration.host = competitions_configuration.host[:
                                                                              -1]

        # Add to the final configuration (required)
        self.set_config_values(kaggle_configuration, config_data)
        self.set_config_values(competitions_configuration, config_data)
        ...
        try:
            self.kaggle_api_client = KaggleApiClient(kaggle_configuration)
            self.kaggle = KaggleServiceApi(self.kaggle_api_client)
    ...
    def set_config_values(self, configuration, config_data):
    ...
    def competitions_list(self,
        ...
        competitions_list_result = self.process_response(
            self.kaggle.competitions_list_with_http_info(group=group or '',
                                                         category=category
                                                         or '',
                                                         sort_by=sort_by or '',
                                                         page=page,
    ...
    def competition_submit(self, file_name, message, competition, quiet=False):
        ...
        if competition is None:
            ...
            url_result = self.process_response(
                self.kaggle.competitions_submissions_url_with_http_info(
                    id=competition,
            ...
            if 'isComplete' in url_result:
                ...
                upload_result = self.process_response(
                    self.kaggle.competitions_submissions_upload_with_http_info(
                        file=file_name,
            ...
            submit_result = self.process_response(
                self.kaggle.competitions_submissions_submit_with_http_info(
                    id=competition,
    ...
    def competition_submissions(self, competition):
        ...
        submissions_result = self.process_response(
            self.kaggle.competitions_submissions_list_with_http_info(
                id=competition))
    ...
    def competition_list_files(self, competition):
        ...
        competition_list_files_result = self.process_response(
            self.kaggle.competitions_data_list_files_with_http_info(
                id=competition))
    ...
    def competition_download_file(self,
        ...
        response = self.process_response(
            self.kaggle.competitions_data_download_file_with_http_info(
    ...
    def competition_leaderboard_download(self, competition, path, quiet=True):
        ...
        """
        response = self.process_response(
            self.kaggle.competition_download_leaderboard_with_http_info(
    ...
    def competition_leaderboard_view(self, competition):
        ...
        result = self.process_response(
            self.kaggle.competition_view_leaderboard_with_http_info(
                competition))
    ...
    def dataset_list(self,
        ...
        datasets_list_result = self.process_response(
            self.kaggle.datasets_list_with_http_info(group=group,
                                                     sort_by=sort_by
                                                     or 'hottest',
                                                     size=size,
                                                     filetype=file_type
    ...
    def dataset_view(self, dataset):
        ...
        result = self.process_response(
            self.kaggle.datasets_view_with_http_info(owner_slug, dataset_slug))
    ...
    def dataset_metadata_update(self, dataset, path):
        ...
        with open(meta_file, 'r') as f:
            ...
            updateSettingsRequest = DatasetUpdateSettingsRequest(
                ...
                data=metadata['data'])
            result = self.process_response(
                self.kaggle.metadata_post_with_http_info(
                    owner_slug, dataset_slug, updateSettingsRequest))
    ...
    def dataset_metadata(self, dataset, path):
        ...
        result = self.process_response(
            self.kaggle.metadata_get_with_http_info(owner_slug, dataset_slug))
    ...
    def dataset_list_files(self, dataset):
        ...
        dataset_list_files_result = self.process_response(
            self.kaggle.datasets_list_files_with_http_info(
    ...
    def dataset_status(self, dataset):
        ...
        dataset_status_result = self.process_response(
            self.kaggle.datasets_status_with_http_info(
                owner_slug=owner_slug, dataset_slug=dataset_slug))
    ...
    def dataset_download_file(self,
        ...
        response = self.process_response(
            self.kaggle.datasets_download_file_with_http_info(
    ...
    def dataset_download_files(self,
        ...
        response = self.process_response(
            self.kaggle.datasets_download_with_http_info(
                owner_slug=owner_slug,
                dataset_slug=dataset_slug,
                _preload_content=False))
    ...
    def dataset_upload_file(self, path, quiet):
        ...
        result = FileUploadInfo(
            self.process_response(
                self.kaggle.datasets_upload_file_with_http_info(
    ...
    def dataset_create_version(self,
        ...
        if id_no:
            result = DatasetNewVersionResponse(
                self.process_response(
                    self.kaggle.datasets_create_version_by_id_with_http_info(
                        id_no, request)))
        else:
            ...
            result = DatasetNewVersionResponse(
                self.process_response(
                    self.kaggle.datasets_create_version_with_http_info(
    ...
    def dataset_create_new(self,
        ...
        result = DatasetNewResponse(
            self.process_response(
                self.kaggle.datasets_create_new_with_http_info(request)))
    ...
    def kernels_list(self,
        ...
        kernels_list_result = self.process_response(
            self.kaggle.kernels_list_with_http_info(
                page=page,
                page_size=page_size,
                group=group,
                user=user or '',
    ...
    def kernels_push(self, folder):
        ...
        result = KernelPushResponse(
            self.process_response(
                self.kaggle.kernel_push_with_http_info(
    ...
    def kernels_pull(self, kernel, path, metadata=False, quiet=True):
        ...
        response = self.process_response(
            self.kaggle.kernel_pull_with_http_info(owner_slug, kernel_slug))
        blob = response['blob']
    ...
    def kernels_output(self, kernel, path, force=False, quiet=True):
        ...
        response = self.process_response(
            self.kaggle.kernel_output_with_http_info(owner_slug, kernel_slug))
        outfiles = []
    ...
    def kernels_status(self, kernel):
        ...
        if '/' in kernel:
            ...
            kernel_slug = kernel
        response = self.process_response(
            self.kaggle.kernel_status_with_http_info(owner_slug, kernel_slug))
        return response
