...
from config.config_converter import (
...
class FieldsConverter:
    def __init__(
        ...
        :param advanced_features: 0 - get sizes of text, 1 - to create partitions , 2 - to crete_sort_fields, 3 - check text_fields_size
        """
    ...
    def __save_ddl_to_file(self, ddl, table_name, ddl_dir):
        """
        ...
        :return:
        """
        f = open(ddl_dir + "/" + self.schema + "_" + table_name + ".sql", "w", )
        f.write(ddl)
        f.close()
    ...
    def create_list_of_tables(
        ...
        for table in tables:
            ...
            if dir_ddl is not None:
                self.__save_ddl_to_file(ddl=sql, table_name=table, ddl_dir=dir_ddl)
