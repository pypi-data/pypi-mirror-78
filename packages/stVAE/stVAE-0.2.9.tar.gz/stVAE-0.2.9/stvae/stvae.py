...
def _cast_data(data, cfg: Config, mode: str, datatype: str = "", preproc: bool = False):
...
class stVAE:
    ...
    def train_classifiers(self, train_data) -> None:
        ...
        self.cfg.classifier_input_dim = self.cfg.input_dim
    ...
    def test(
        ...
        pretrained = None
        if not self.celltype_clf is None and not self.form_clf is None:
            pretrained = (self.celltype_clf,
                          self.form_clf)
        if self.celltype_clf is None or self.form_clf is None:
            ...
            return test(
                ...
                batch_indices,
                labels,
                pretrained_classifiers=pretrained
            )
        else:
            if not classifier_train_data is None:
                warn(
