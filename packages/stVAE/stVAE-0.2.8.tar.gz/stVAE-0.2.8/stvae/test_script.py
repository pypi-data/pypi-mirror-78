...
def train_classifiers(
    cfg, annot_dataloader, count_celltypes, count_classes, track=False, test_data=None
):
    device = torch.device("cuda" if cuda_is_available() and cfg.use_cuda else "cpu")
    celltype_clf = Classifier(
        inp_size=cfg.classifier_input_dim, out_size=count_celltypes
    )
    ...
    celltype_clf_opt = torch.optim.Adam(
        celltype_clf.parameters(),
        weight_decay=cfg.celltype_clf_wdecay,
        lr=cfg.celltype_clf_lr,
    )
    form_clf_opt = torch.optim.Adam(
        form_clf.parameters(), weight_decay=cfg.form_clf_wdecay, lr=cfg.form_clf_lr
    )
...
def test(
    ...
    target_classes_transfer = np.zeros(
        (class_ohe_test.shape[0] * class_ohe_test.shape[1], class_ohe_test.shape[1])
    )
    target_classes_transfer[
        np.arange(target_classes_transfer.shape[0]),
        np.arange(target_classes_transfer.shape[0]) % target_classes_transfer.shape[1],
    ] = 1
    ...
    res_norm = (
        (residual_transfer ** 2).mean(1).reshape((-1, cfg.count_classes))
    )  # l2 norm,
    res_equal = class_ohe_test.argmax(1)
    res_equal = (
        res_equal
        if isinstance(res_equal, np.ndarray)
        else res_equal.cpu().detach().numpy()
    )
    ...
    if pretrained_classifiers is None:
        ...
        latents_dataloader = torch.utils.data.DataLoader(
            latents_dataset, batch_size=cfg.batch_size, shuffle=True, drop_last=True
        )
        celltype_clf_latents, form_clf_latents = train_classifiers(
            cfg, latents_dataloader, celltype_test.shape[1], cfg.count_classes
        )
    else:
        celltype_clf_expr = pretrained_classifiers[0]
        form_clf_expr = pretrained_classifiers[1]
    ...
    ebm_score = {}
    ebm_score["test"] = [0.0 for i in range(1)]
    ebm_score["transfered"] = [0.0 for i in range(1)]
    for n in range(1):
    ...
    if not annot_dataloader is None:
        celltype_train_labelenc = (
            annot_dataloader.dataset.tensors[2].argmax(1).cpu().numpy()
        )
        form_train_labelenc = (
            annot_dataloader.dataset.tensors[1].argmax(1).cpu().numpy()
        )
    ...
    with torch.no_grad():
        ...
        if not annot_dataloader is None:
            ...
            predicted_form_train = form_clf_expr(annot_expression)

        predicted_form_test = form_clf_expr(transfer_expression_tensor)
        predicted_form_transfer = form_clf_expr(transfer_expression_tensor)
        # predict celltypes and batch indices by model's latents
        predicted_celltype_test_latents = None
        predicted_celltype_transfer_latents = None
        predicted_form_latents = None
        if not celltype_clf_latents is None:
        ...
        predicted_celltype_test_labelenc = (
            predicted_celltype_test.argmax(1).cpu().detach().numpy()
        )
        predicted_celltype_test_raw_labelenc = (
            predicted_celltype_test_raw.argmax(1).cpu().detach().numpy()
        )
        predicted_form_transfer_labelenc = (
            predicted_form_transfer.argmax(1).cpu().detach().numpy()
        )
        predicted_celltype_train_labelenc = None
        if not predicted_celltype_train is None:
            predicted_celltype_train_labelenc = (
                predicted_celltype_train.argmax(1).cpu().detach().numpy()
            )
            predicted_form_train_labelenc = (
                predicted_form_train.argmax(1).cpu().detach().numpy()
            )

    sameclass_mask = (
        (init_classes_transfer.argmax(1) == target_classes_transfer.argmax(1))
        .cpu()
        .numpy()
        .astype("bool")
    )
    ...
    form_train_accuracy = None
    if not predicted_celltype_train_labelenc is None:
        celltype_train_accuracy = (
            predicted_celltype_train_labelenc == celltype_train_labelenc
        ).mean()
        form_train_accuracy = (
            predicted_form_train_labelenc == form_train_labelenc
        ).mean()
    celltype_test_raw_accuracy = (
        predicted_celltype_test_raw_labelenc == np.array(celltype_test_raw_labelenc)
    ).mean()
    celltype_test_accuracy = (
        predicted_celltype_test_labelenc == celltype_test_labelenc
    ).mean()

    celltype_notransfer_accuracy = (
        predicted_celltype_test_labelenc[sameclass_mask] == celltype_test_labelenc[sameclass_mask]
    ).mean()
    form_notransfer_accuracy = (
        predicted_form_transfer_labelenc[sameclass_mask] == form_test_labelenc[sameclass_mask]
    ).mean()
    ...
    if (~sameclass_mask).any():
        ...
        pred_f_t = (
            predicted_form_test
            if isinstance(predicted_form_test, np.ndarray)
            else predicted_form_test.argmax(1).cpu().detach().numpy()
        )
        rep = classification_report(
    ...
    transfer_confusion = confusion_matrix(
        form_test_labelenc[~sameclass_mask], pred_f_t[~sameclass_mask], normalize="all"
    )
    if cfg.verbose in ("all", "test"):
    ...
    rep = classification_report(
        form_test_labelenc[~sameclass_mask], pred_f_t[~sameclass_mask], output_dict=True
    )
    df = pd.DataFrame(rep).transpose()
    if cfg.verbose in ("all", "test"):
    ...
    celltype_notransfer_latents_accuracy = None
    celltype_transfer_latents_accuracy = None
    if not predicted_celltype_test_latents is None:
        predicted_celltype_latents_labelenc = (
            predicted_celltype_test_latents.argmax(1).cpu().detach().numpy()
        )
        predicted_celltype_transfer_latents_labelenc = (
            predicted_celltype_transfer_latents.argmax(1).cpu().detach().numpy()
        )
        ...
        celltype_notransfer_latents_accuracy = (
            np.array(predicted_celltype_latents_labelenc) == np.array(celltype_test_raw_labelenc)
        ).mean(0)
        if (~sameclass_mask).any():
    ...
    if cfg.verbose in ("all", "test"):
        print("On expression:")
        print(
            "\tCell type prediction accuracy [vae transfered data]",
            celltype_test_accuracy,
            "< with the same class transfer",
        )
        print(
            "\tCell type prediction accuracy [reconstructed data]",
            celltype_notransfer_accuracy,
        )
        print(
    ...
    return {
        ...
        "Class prediction expression [train]": form_train_accuracy,
        "Class prediction accuracy expression [test hold out data]": form_test_accuracy,
        "Class prediction accuracy expression [reconstructed data]": form_notransfer_accuracy,
        "Class prediction accuracy expression [transfered to another classes]": form_transfer_accuracy
