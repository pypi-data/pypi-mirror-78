...
class tree: ## tree class
    ...
    def collapseSubtree(self,cl,givenName,verbose=False,widthFunction=lambda k:len(k.leaves),redraw=True):
        ...
        remove_from_tree=self.traverse_tree(cl,include_condition=lambda k: True,reset_branches=False)
        collapsedClade.subtree=remove_from_tree
        assert len(remove_from_tree)<len(sum([k.subtree if isinstance(k,clade) else [k] for k in self.Objects],[])),'Attempted collapse of entire tree'
