...
extern "C" {
#endif
PETSC_EXTERN void  tsadapthistorygetstep_(TSAdapt adapt,PetscInt *step,PetscReal *t,PetscReal *dt, int *__ierr){
*__ierr = TSAdaptHistoryGetStep(
	(TSAdapt)PetscToPointer((adapt) ),*step,t,dt);
}
PETSC_EXTERN void  tsadapthistorysethistory_(TSAdapt adapt,PetscInt *n,PetscReal hist[],PetscBool *backward, int *__ierr){
*__ierr = TSAdaptHistorySetHistory(
	(TSAdapt)PetscToPointer((adapt) ),*n,hist,*backward);
}
PETSC_EXTERN void  tsadapthistorysettrajectory_(TSAdapt adapt,TSTrajectory tj,PetscBool *backward, int *__ierr){
