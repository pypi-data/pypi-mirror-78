...
class FileProfile:
            ...
            for i, filename in enumerate(self.filenames):
                if "groups" in filename:
                    for grp in filename.get("groups"):
                        ...
                        self.add_field(DeIdField.factory(grp_tmp, dry=True))
    ...
    def _add_identity_fields(self, filename):
        ...
        filename['input-regexp'] are set as identity field"""
        regex = re.compile(r"\{([^}]+)\}")
    ...
    def validate(self, enhanced=False):
        ...
        if self.date_format:
            errors += self.validate_datetime_format_code(self.date_format)
        if self.datetime_format:
            errors += self.validate_datetime_format_code(self.datetime_format)
        ...
        for fieldname, field in self.field_map.items():
            if isinstance(field, DeIdIncrementDateTimeField):
                if field.datetime_format != self.datetime_format:
                    errors += self.validate_datetime_format_code(field.datetime_format)

            if isinstance(field, DeIdIncrementDateField):
                if field.date_format != self.date_format:
                    errors += self.validate_datetime_format_code(field.date_format)
