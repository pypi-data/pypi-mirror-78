...
import logging
import urllib3
from xmaios.monitor.BotMonitor import BotMonitor
from xmaios.Certificate import Certificate
from xmaios.Intercept import Intercept
from xmaios.Request import Request
from xmaios.Response import Response
from xmaios.Base import Base
from xmaios.Utils import Utils
from xmaios.Constants import constants
...
class Bot(object):
    ...
    def set_should_open_microphone(self, val=True):
        """
        设置是否开启麦克风
        :return:
        """
        if self.__response:
            self.__response.set_should_open_microphone(val)
    ...
    def get_delegate_slot(self):
        """
        委托情况下优先追问的词槽
        :return:
        """
        return self.get_slots('DelegateAskSlot')

    def set_delegate(self, slot_token=None):
        """
        设置delegate 某个槽位或确认意图
        :return:
        """
        if slot_token:
            self.set_slots('DelegateAskSlot', slot_token)
    ...
    def add_intent_handler(self, intent_token, func):
        """
        添加对特定意图的处理函数
        :param intent_token:  意图英文标识名
        :param func:    回调方法
        :return:
        """

        return self.__add_handler(intent_token, func)
    ...
    def get_intent_token(self):
        """
        获取第一个Intent的名字
        :return:
        """

        return self.__nlu.get_intent_token() if self.__nlu else ''
    ...
    def get_slots(self, field, index=0, all_values=False):
        """
        获取槽位值
        :param field:   槽位名
        :param index:   槽位 位置 默认值为0
        :param all_values:   返回所有值 默认为False
        :return:
        """

        if self.__nlu and field and isinstance(field, str):
            return self.__nlu.get_slot(field, index, all_values)
    ...
    def update_storage(self, storage):
        if self.__response:
            self.__response.update_storage(storage)
    ...
    def __check_handler(self, handler):
        ...
        if re.match(rg['intent'], handler):
            if self.get_intent_token() == handler:
                self.__callback_data = None
                return True
            else:
                self.__callback_data = {'type': 'intent', 'message': 'handler未匹配到:%s' % self.get_intent_token()}
