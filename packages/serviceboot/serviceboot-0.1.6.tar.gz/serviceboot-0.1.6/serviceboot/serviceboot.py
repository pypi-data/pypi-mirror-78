...
class WebSocketSever(tornado.websocket.WebSocketHandler):
    ...
    def on_message(self, message):
        ...
        if msg.get('type') == 'set_conn_id':
            self.conn_id = msg.get('content')
            if self.conn_id is not None:
                old = self.opened_connections.get(self.conn_id)
                if old is None:
                    self.opened_connections[self.conn_id] = self
                    logging.critical('WebSocket connection opened: {}'.format(self.conn_id))
                    self.write_message({
                        'type': 'welcome',
                        'content': 'WebSocket connection opened: {}'.format(self.conn_id),
                    })
            else:
                logging.error('WebSocket conn_id error')
                self.close()
        else:
            receiver = msg.get('receiver')
            # 未指定receiver或receiver第一个字符为'/'
            if receiver is None or receiver[0] == '/':
                return
            if '/' in receiver:
                i = receiver.find('/')
                msg['receiver'] = receiver[i + 1:]
                receiver = receiver[0: i]

            # 转发消息
            next_websocket = self.opened_connections.get(receiver)
            if next_websocket:
                msg['sender'] = self.conn_id
                next_websocket.write_message(msg)

    def on_close(self):
        self.opened_connections.pop(self.conn_id)
        logging.critical('WebSocket connection closed: {}'.format(self.conn_id))
