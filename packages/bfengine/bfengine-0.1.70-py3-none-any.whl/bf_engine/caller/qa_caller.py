...
class QACaller(CallerBase):
    ...
    def get_similar_qa_list(self,user_inputs:str,max_topn,category_ids):
        similar_sq_result = []
        sq_list = [];
        ans_list = [];
        sq_map = {};
        data = {"search":[{"keyword":"","field":"sq"}],"conditions":{"category":{"id_list":category_ids,"logical_op":"and"}}}
        ...
        for match_sq in resp['data']:
            if match_sq['sqId'] in sq_list:
               match_sq['category_id'] = sq_map.get(match_sq['sqId'])
               ans_resp = requests.get(self.upload_faq_json_sq_url + "/" + match_sq['sqId']+"?field=answer", headers=self.header, json={}).json()
