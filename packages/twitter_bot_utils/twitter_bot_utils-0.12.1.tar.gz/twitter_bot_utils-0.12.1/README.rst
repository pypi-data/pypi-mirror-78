...
If you have more than one bot, a simple setup is to have one app for
each bot. Visit `apps.twitter.com <https://apps.twitter.com>`__,
register the app, and then choose "Create my access token" in the "keys
and tokens" tab.
...
If you have one app shared by several bots, create an ``apps`` section
in the config file:
...
``bots.yaml`` file set up like this:
