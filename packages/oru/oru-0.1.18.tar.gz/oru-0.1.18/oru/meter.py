...
class Meter(object):
    ...
    Attributes:
        ...
        mfa_aswer: A string representing the multiple factor authorization answer for the account
        account_id: A string representing the account's id
        meter_id: A string representing the meter's id
    """

    def __init__(self, email, password, mfa_aswer, account_id, meter_id, loop=None):
        """Return a meter object whose meter id is *meter_id*"""
        self.email = email
        self.password = password
        self.mfa_aswer = mfa_aswer
    ...
    def last_read(self):
        ...
        try:
            if self.loop is None:
                asyncio.get_event_loop().run_until_complete(self.browse())
            else:
                asyncio.set_event_loop(self.loop)
                asyncio.ensure_future(self.browse())
            ...
            self.last_read_wh = lastRead['value']
            self.unit_of_measurement = jsonResponse['unit']

            _LOGGER.debug("val = %s %s", self.last_read, self.unit_of_measurement)
    ...
    async def browse(self):
        ...
        await page.type("#form-login-mfa-code", self.mfa_aswer)
