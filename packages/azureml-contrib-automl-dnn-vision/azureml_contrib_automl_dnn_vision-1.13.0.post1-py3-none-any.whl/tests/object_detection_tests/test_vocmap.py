...
class TestVocMap:
    ...
    def test_map(self):
        ...
        label_map_score = vocmap._map([], label3)
        assert label_map_score == -1.0
        # Non non crowd gts, no predictions
        label_map_score = vocmap._map([], label2)
        assert label_map_score == -1.0

        # Non gts, some predictions
        predictions = [
            {"image_id": "1", "bbox": [0, 0, 299, 199], "category_id": label3, "score": 0.5}
        ]
        label_map_score = vocmap._map(predictions, label3)
        assert label_map_score == -1.0
        # No non crowd gts, some predictions
        predictions = [
            {"image_id": "1", "bbox": [0, 0, 299, 199], "category_id": label2, "score": 0.5}
        ]
        label_map_score = vocmap._map(predictions, label2)
        assert label_map_score == -1.0

        # Has Non crowd gts, no predictions
        label_map_score = vocmap._map([], label1)
        assert label_map_score == 0.0
        ...
        label_map_score = vocmap._map(predictions, label1)
        assert label_map_score.ndim == 0  # Scalar
        assert label_map_score != -1
