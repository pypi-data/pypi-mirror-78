...
class StepLRWrapper(BaseLRSchedulerWrapper):
    ...
    def __init__(self, optimizer, **kwargs):
        ...
        lr_scheduler_parameters = learning_parameters.StepLRSchedulerParameters(**kwargs)

        self._step_size = (lr_scheduler_parameters.step_size if
                           lr_scheduler_parameters.step_size is not None else
                           constants.SchedulerParameters.DEFAULT_STEP_LR_STEP_SIZE)

        self._gamma = (lr_scheduler_parameters.gamma if
                       lr_scheduler_parameters.gamma is not None else
...
class WarmUpCosineLRWrapper(BaseLRSchedulerWrapper):
    ...
    def __init__(self, optimizer, **kwargs):
        ...
        lr_scheduler_parameters = learning_parameters.WarmUpCosineLRSchedulerParameters(**kwargs)

        if lr_scheduler_parameters.warmup_steps is None:
            raise AutoMLVisionSystemException("warmup_steps is missing from learning_parameters. "
                                              "Cannot use warmUp cosine lr scheduler.")
        if lr_scheduler_parameters.total_steps is None:
            raise AutoMLVisionSystemException("total_steps is missing from learning_parameters. "
                                              "Cannot use warmUp cosine lr scheduler.")

        self._warmup_steps = lr_scheduler_parameters.warmup_steps
        self._total_steps = lr_scheduler_parameters.total_steps
        self._cycles = (lr_scheduler_parameters.cycles if
                        lr_scheduler_parameters.cycles is not None else
                        constants.SchedulerParameters.DEFAULT_WARMUP_COSINE_LR_CYCLES)
