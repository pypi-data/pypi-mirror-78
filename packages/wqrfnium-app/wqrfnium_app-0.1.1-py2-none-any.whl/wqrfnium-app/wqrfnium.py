...
try:
    ...
    cfp.read('wqrfnium-app.ini')
    elements_xls_path = cfp.get('Excel','elements_xls_path')
except: # create wqrfnium-app.ini
    cfp = configparser.ConfigParser()
    cfp["Excel"] = {"elements_xls_path":""}
    with open('wqrfnium-app.ini','w') as fp:
        cfp.write(fp)
    elements_xls_path = cfp.get('Excel','elements_xls_path')
...
def begin_wqrf(path):
    ...
    with open("wqrfnium-app.ini","w+") as f:
        cfp.write(f)
...
if elements_xls_path == '': #no path
    ...
    print('You are first use wqrfnium-app,it is creating elements.xls,you must edit elements.xls and play wqrfnium-app after!')
