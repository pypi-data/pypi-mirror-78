...
def build(hub, bname: str):
    ...
    # Run fpm
    config = _prep_tmpdir(
        hub, opts.name, root, opts.pkg, opts.pkg_tgt, opts.dir, opts.onedir,
    )
    cmd = _get_cmd(
        opts.name,
        root,
...
def _run_fpm(hub, cmd: List[str]):
    """
    :param cmd:
    """
    retcode = subprocess.call(cmd)
    if retcode != 0:
        hub.log.error(f"rpm-build failed. fpm exited with return code: {retcode}")
        sys.exit(retcode)
