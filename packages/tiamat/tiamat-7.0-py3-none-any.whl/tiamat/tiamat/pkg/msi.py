...
def _gen_wixobj(candle_path: str, wxs_file: str):
    """
    :param candle_path:
    :param wxs_file:
    """
    ...
    if ret.returncode != 0:
        raise WIXError(
            f"wixobj generation failed!\nret:{ret.returncode}\nstdout: {ret.stdout}\nstderr: {ret.stderr}\n"
        )


def _build_msi(light_path: str, wixobj_file: str):
    ret = subprocess.run([os.path.join(light_path, "light.exe"), wixobj_file])
    if ret.returncode != 0:
        raise WIXError(
            f"mis generation failed!\nret:{ret.returncode}\nstdout: {ret.stdout}\nstderr: {ret.stderr}\n"
        )


def _clean_file(opts, file: str):
    try:
        os.remove(file)
    except OSError:
        pass
