...
class GBMFitter:
    ...
    def fit(self, X, y, categorical=None, feature_selection=-1, skip_CB=False, skip_XGB=False, skip_LGB=False,
        ...
        if not skip_CB:
            self._fit('CB', X, y, categorical, feature_selection, early_stopping)

        if not skip_XGB:
            self._fit('XGB', X, y, categorical, feature_selection, early_stopping)

        if not skip_LGB:
            self._fit('LGB', X, y, categorical, feature_selection, early_stopping)

    def _fit(self, library, X, y, categorical=None, feature_selection=-1, early_stopping=None):
    ...
    def _train(self, library, model_data, train, validation=None, categorical=[], early_stopping=None):
        ...
        if library == 'XGB':
            if categorical:
                X_train = ce.one_hot.OneHotEncoder(cols=categorical, drop_invariant=True).fit_transform(X_train)
