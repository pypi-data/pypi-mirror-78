...
class ConfigCmd:
    ...
    def write_exp_config(self, exp, vconf):
        ...
        # Set input data path.
        path_data = new_xml.find(".//path//data")
        path_data_attr = ""
        if (len(path_data.attrib) > 0):
            if "absolute" in path_data.attrib:
                path_data_attr = path_data.attrib["absolute"]
        if self._target and path_data_attr != "true":
            path_data.text = self._target.replace("\\", "/").replace("'\\'", "/") + "/" + path_data.text.replace("\\","/").replace("'\\'","/")  # os.path.join(self._target, path_data.text) #self._target + "/" + path_data.text #str(Path(self._target) / Path(path_data.text))
            etree.tostring(new_xml)
