...
try:
    ...
    from oitnb._oitnb_lib2to3 import pygram, pytree
    from oitnb._oitnb_lib2to3.pgen2 import driver, token
    from oitnb._oitnb_lib2to3.pgen2.grammar import Grammar
    from oitnb._oitnb_lib2to3.pgen2.parse import ParseError
except ImportError:
    ...
    from _oitnb_lib2to3.pgen2.grammar import Grammar
...
quotes = ["'", '"', "'''", '"""']
messages = dict(
    NO_SRC=["No Path provided. Nothing to do", ".", " 😴"],
    NO_PATHS_GIVEN=["No Python files are present to be formatted. Nothing to do", ".", " 😴"],
    BANG_OK=["", "All OK!", "All done! ✨ 🍰 ✨"],
    BANG_NOT_OK=["", "Errors encountered", "Oh no! 💥 💔 💥"],
)
post_processors = []
issues_url = "https://sourceforge.net/p/oitnb/tickets/"
...
def read_package_data(
    ...
    otherwise.
    """
    from oitnb.parseconfig import literal_eval, _package_data_parser
    ...
    if not value:
        ...
        if user_data.is_file():
            ...
            value.append(str(user_data))

        root = find_project_root(ctx.params.get("src", ()))
        path = root / "__init__.py"
        if path.is_file():
            # take the 'oitnb' section from the package data
            config.update(
                _package_data_parser(path).get(_package_data["full_package_name"], {})
            )
            value.append(str(path))
        value = ' + '.join(value)
    else:
        config_path = Path(value)
        assert config_path.is_file()
        config = literal_eval(config_path.read_text())

    if not config:
        return value + ' (no config data found)'
...
def main(
    ...
    if verbose or not quiet:
        out(messages["BANG_NOT_OK" if report.return_code else "BANG_OK"][0])
...
class BracketTracker:
    ...
    def mark(self, leaf: Leaf) -> None:
        ...
        If a leaf is a delimiter (a token on which oitnb can split the line if
...
def assert_equivalent(src: str, dst: str) -> None:
    ...
    try:
        ...
        raise AssertionError(
            f"INTERNAL ERROR: oitnb produced invalid code: {exc}. "
            f"Please report a bug on {issues_url}.  "
    ...
    if src_ast_str != dst_ast_str:
        ...
        raise AssertionError(
            f"INTERNAL ERROR: oitnb produced code that is not equivalent to "
            f"the source.  "
            f"Please report a bug on {issues_url}.  "
