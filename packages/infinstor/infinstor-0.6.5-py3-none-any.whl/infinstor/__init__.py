...
def get_mlflow_run_artifacts_info(run_id):
    ...
    rundata = run.data
    runinfo = run.info
    info('>>>>>>>>>>>> rundata=' + str(rundata))
    info('>>>>>>>>>>>> runinfo=' + str(runinfo))
    artifact_uri = runinfo['artifact_uri']
    ...
    if (len(runs) == 0):
        ...
        for run in runs:
            if (run.info.status == 'FINISHED'):
                return True, run.info.run_id, cache_key
        return True, runs[0].info.run_id, cache_key
    

def run_transform_inline(service_name, input_data_spec, xformname, cache_key, **kwargs):
    ...
    with start_run() as run:
        ...
        return run_transform_string_inline(client, bucketname, prefix_in, transform_string, False,\
            **kwargs)
...
def run_transform_singlevm(service_name, input_data_spec, xformname, instance_type, cache_key,\
        **kwargs):
...
def run_transform(run_options, **kwargs):
    ...
    found, run_id, cache_key = check_in_cache(input_data_spec, xformname)
    if (found == True):
    ...
    if (run_location == "inline"):
        return run_transform_inline(service_name, input_data_spec, xformname, cache_key, **kwargs)
