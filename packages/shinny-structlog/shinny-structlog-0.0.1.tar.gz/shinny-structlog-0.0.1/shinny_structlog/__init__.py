...
class ShinnyLogger(logging.Logger):
    ...
    def kwargs_to_extra(self, **kwargs):
        """
        将 kwargs 除 ["exc_info", "extra", "stack_info"] 之外的参数添加到 extra 字段上，
        然后将 extra 的值放在 extra["extra"] 下
        """
        extra = {k: kwargs[k] for k in kwargs if k not in ["exc_info", "extra", "stack_info"]}
        if extra:
            for k in extra.keys():
                del kwargs[k]
            _extra_val = deepcopy(kwargs.get("extra")) if kwargs.get("extra") else {}
            _extra_val.update(extra)
            kwargs["extra"] = {"extra": _extra_val}
        return kwargs
...
class JSONFormatter(logging.Formatter):
    ...
    def __init__(self, log_keys: list = None):
        self.log_keys = set(log_keys) if log_keys else set()
        self.log_keys = self.log_keys & _VALID_ATTRS
