...
from solconfig.broker import CmdOption, Broker
...
def cli(ctx, admin_user, admin_password, host, curl_only, insecure, ca_bundle):
    ...
    cmd_option = CmdOption(admin_user, admin_password, host, curl_only, insecure, ca_bundle, False)
    broker = Broker(cmd_option)
...
def backup(ctx, object_type, object_names, reserve_default_value, reserve_deprecated, opaque_password):
    ...
    broker.option.set_opaque_password(opaque_password)
    result = broker.backup(TYPE_2_COLL_NAME[object_type], 
...
def delete(ctx, object_type, object_names):
    ...
    broker = ctx.obj['BROKER']
    broker.fetch_spec_and_api()
    broker.delete(TYPE_2_COLL_NAME[object_type], object_names)
