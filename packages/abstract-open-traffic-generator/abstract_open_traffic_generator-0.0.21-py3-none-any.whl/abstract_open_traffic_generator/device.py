...
class Device(object):
    ...
    - ethernet (list[Ethernet]): TBD
    """
    def __init__(self, name=None, devices_per_port=None, parent=None, ethernet=[]):
        if isinstance(name, (str, type(None))) is True:
            self.name = name
        else:
            raise TypeError('name must be an instance of (str, type(None))')
        ...
        if isinstance(ethernet, (list, type(None))) is True:
            self.ethernet = ethernet
        else:
            raise TypeError('ethernet must be an instance of (list, type(None))')
