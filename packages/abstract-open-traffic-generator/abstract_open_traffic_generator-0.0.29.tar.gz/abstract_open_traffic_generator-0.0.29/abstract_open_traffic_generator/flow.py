...
class Flow(object):
    ...
    - state (Union[start, stop, pause, enable, disable]): Set the configured state of the flow. Use the /results API to get the actual state of the flow
    """
    def __init__(self, name=None, endpoint=None, packet=[], size=None, rate=None, duration=None, state='stop'):
...
class DeviceEndpoint(object):
    ...
    Args
    ----
    - tx_devices (list[str]): The unique names of emulated devices that will be transmitting. The devices names can be 1..n of DeviceGroup, Device, Ethernet, Vlan, Ipv4 etc
    - rx_devices (list[str]): The unique names of emulated devices that will be receiving. The devices names can be 1..n of DeviceGroup, Device, Ethernet, Vlan, Ipv4 etc
...
class Rate(object):
    ...
    - gap (int): The minimum gap in bytes between packets
    """
...
class Fixed(object):
    ...
    Args
    ----
    - delay (int): Start transmit of the flow after a delay of this number of bytes
    ...
    """
    def __init__(self, delay=None, packets=0):
...
class Burst(object):
    ...
    Args
    ----
    - gap (int): The gap between each burst as a number of bytes
