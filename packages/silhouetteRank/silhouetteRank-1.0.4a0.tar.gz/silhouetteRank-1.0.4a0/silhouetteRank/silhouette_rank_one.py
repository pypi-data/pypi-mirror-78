...
def main():
	...
	os.system("cat args.basic|%s/parallel --jobs %d --max-args=2 %s/silhouette_rank_main -x %s -c %s -r {1} -e {2} -m %s -o . \"2>\" %s/real_{1}_{2}.out" % (args.parallel_path, args.num_core, bin_path, args.expr, args.centroid, args.matrix_type, args.logdir))
	...
	os.system("cat args|%s/parallel --jobs %d --max-args=3 %s/silhouette_rank_random -r {1} -e {2} -m %s -o . -q {3} \"2>\" %s/{1}_{2}_{3}.out" % (args.parallel_path, args.num_core, bin_path, args.matrix_type, args.logdir))
