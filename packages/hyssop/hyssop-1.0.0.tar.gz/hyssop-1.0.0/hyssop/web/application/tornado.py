...
This module provicde the classes integrate tornado application and clients.

    - TornadoApplication: add component-based architecture to tornado application
    ...
    - StreamUploadClient is the data uploader via streaming http request. for example:
...
Modified By: hsky77
Last Updated: September 4th 2020 14:23:47 pm
'''
...
from ...util import FunctionLoopWorker, BaseLocal
from ..constants import LocalCode_Connect_Failed, KB
from .base import ComponentApplicationMixin


class TornadoApplication(ComponentApplicationMixin, Application):

    def init_application_mixin(self, settings: Dict[str, Any]) -> None:
        from ..component import add_module_default_logger

        # add tornado logger to the default loggers of LoggerComponent
        add_module_default_logger(
            ['tornado.access', 'tornado.application', 'tornado.general'])

        super().init_application_mixin(settings)
        ...
        if self.conn is None:
            ...
            if not self.is_connected:
                raise RuntimeError(BaseLocal.get_message(
                    LocalCode_Connect_Failed, self.url))
...
class StreamUploadClient():
    def send_file(self, url: str, source: str, target: str, chunk_size: int = 4*KB,
        ...
        return self._send_file(url, source, target, chunk_size, on_chunk_send)
    ...
    def send_bytes(self, url: str, data: ByteString, chunk_size: int = 4*KB,
        ...
        return self._send_bytes(url, data, chunk_size, on_chunk_send, encoding)
    ...
    def send_by_generator(self, url: str, generator: Generator, encoding: str = None) -> requests.Response:
        return self._send_by_generator(url, generator, encoding)
