...
class Separatrice:
  ...
  # split by comma and check which pieces are true clauses 
  def split_by_comma(self,text):
    ...
    '''
    result = []
    cands = [cand for cand in re.split(',',text) if cand != ' ']
  ...
  def into_clauses(self,text):
    ...
    for clause in clauses:
      for x in self.split_by_comma(clause):
        temp.append(x.strip(' '))
