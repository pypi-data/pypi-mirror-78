...
namespace anyks {
	...
	namespace Methods {
		...
		void setOption(const alm_t::options_t option) noexcept {
			...
			// Если установлен режим отладки
			if(option == alm_t::options_t::debug)
				// Устанавливаем режим отладки для токенизатора
				tokenizer.setOption(tokenizer_t::options_t::debug);
		}
		/**
		 * setOption Метод установки опций модуля для токенизатора
		 * @param option опция для установки
		 */
		void setOption(const tokenizer_t::options_t option) noexcept {
			// Выполняем установку опций модуля
			tokenizer.setOption(option);
		}
		/**
...
PYBIND11_MODULE(alm, m) {
	...
	.value("stress", anyks::tokenizer_t::options_t::stress)
	.value("uppers", anyks::tokenizer_t::options_t::uppers)
	.value("collect", anyks::tokenizer_t::options_t::collect)
	 ...
	 */
	{
		...
		.def("setOption", overload_cast_ <const anyks::alm_t::options_t> ()(&anyks::Methods::setOption), "Method for set module options")
		// arabic2Roman Метод перевода арабских чисел в римские
		...
		// setOption Метод установки опций модуля токенизатора
		.def("setOption", overload_cast_ <const anyks::tokenizer_t::options_t> ()(&anyks::Methods::setOption), "Method for set module options")
		// isSuffix Метод проверки слова на суффикс цифровой аббревиатуры
