...
class MainAppBase(ConsoleApp, ABC):
    ...
    def widget_by_flow_id(self, flow_id: str) -> Optional[Any]:
        ...
        :param flow_id:         flow id value of the searched widget's `ae_flow_id` attribute.
        :return:                widget that has a `ae_flow_id` attribute with the value of the passed flow id.
        """
        return self.widget_by_attribute('ae_flow_id', flow_id)

    def widget_by_state_name(self, state_name: str) -> Optional[Any]:
        """ determine the widget having the passed app state name (ae_state_name).

        :param state_name:      app state name of the widget's `ae_state_name` attribute.
        :return:                widget that has a `ae_state_name` attribute with the passed app state name.
        """
        return self.widget_by_attribute('ae_state_name', state_name)
