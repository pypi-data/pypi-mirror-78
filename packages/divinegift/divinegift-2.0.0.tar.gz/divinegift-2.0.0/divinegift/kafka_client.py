...
class KafkaClient:
    ...
    def set_offset(self, topic, partitition, offset):
        if self.consumer:
            topic_p = TopicPartition(topic, partitition, offset)
    ...
    def read_messages(self, topic, partitition=0, offset=OFFSET_STORED):
        if self.consumer:
            self.consumer.subscribe([topic])
            self.set_offset(topic, partitition, offset)
