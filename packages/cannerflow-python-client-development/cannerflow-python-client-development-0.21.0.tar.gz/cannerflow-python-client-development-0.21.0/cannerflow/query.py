...
class Query(object):
    ...
    def __iter__(self):
        ...
        # fetch first page
        result = self.request.post(gen_sql_result_payload(self.id, self.limit, self.limit * (page - 1))).get(
            'sqlResultPagination')
        data = result['result']
        ...
        while (page - 1) * self.limit + index < self.row_count:
            if index == self.limit:
                # fetch new page
                result = self.request.post(gen_sql_result_payload(self.id, self.limit, self.limit * (page - 1))).get('sqlResultPagination')
                data = result['result']
                index = 0
                page += 1
            if len(data) > index:
