...
def create_dbconn():
    try:
        conn = psycopg2.connect(host=os.getenv("PGHOST", '127.0.0.1'), user=os.environ["USER"], password=os.environ["PASSWORD"], port=os.getenv('PGPORT', 5432), database=os.getenv('database', 'postgres'))
    except Exception:
        logging.error(
