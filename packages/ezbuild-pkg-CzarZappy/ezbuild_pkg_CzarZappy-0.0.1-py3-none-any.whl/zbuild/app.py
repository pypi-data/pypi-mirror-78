import zbuild.common.file as file
import sys, getopt
from pathlib import Path

from zbuild.common import bash, butler
import zbuild.common.unity as unity
from zbuild.common.steamcmd import SteamCMD
...
def run(argv):
    ...
    project_settings_folder = Path("ProjectSettings/")
    project_version_path = project_settings_folder / "ProjectVersion.txt"
    ...
    print('Using Unity:', unity_path)

    Unity = unity.Unity(unity_path, output_dir)
    steam_cmd = SteamCMD()
    ...
    print("Publishing to ITCH.IO: {}".format(project_name))
    ...
    for target_config in build_config["targets"]:
        ...
        if build_config["steps"].__contains__("build"):
            ...
            Unity.run_build_target(project_name, build_target, project_output_dir)
