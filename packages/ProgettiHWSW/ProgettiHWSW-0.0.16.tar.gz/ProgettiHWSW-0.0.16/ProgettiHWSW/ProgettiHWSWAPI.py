...
class ProgettiHWSWAPI:
	...
	async def get_switches(self):
		...
		for i in relay_tags:
			...
			states[number if is_old_board is False else number + 1] = True if i.text == "1" else False
		return states

	async def get_inputs(self):
		"""Return all switch states."""
		request = await self.api.request("get", STATUS_XML_PATH)
		if request == False:
			return False
		...
		for i in relay_tags:
			...
			states[number if is_old_board is False else number + 1] = True if i.text == "up" else False
		return states
