...
push_place_front_photo_query = """
  mutation pushPlaceFrontPhoto(
    $uuid: UUID!,
    $frontPhoto: String!
  ) {
    pushPlaceFrontPhoto(input: {
      uuid: $uuid,
      frontPhoto: $frontPhoto
    }) {
      place {
...
class PlaceAPI(API):
    ...
    @API.expose_method
    def push_place_front_photo(self, uuid, front_photo):
        """
        Send through bsecure_client.File object with the place UUID.
        """
        front_photo_file_id = self.upload_front_photo(front_photo)
        response_data = self.perform_query(
            push_place_front_photo_query,
        ...
        return response_data['pushPlaceFrontPhoto']['place']['frontPhoto']
