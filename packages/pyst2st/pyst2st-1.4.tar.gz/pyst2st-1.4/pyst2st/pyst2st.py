    ...
    def get_graduation_status(self):
        ...
        if self.data['grad_info']:
            ...
            if len(date) > 9:
                ...
                month = int(date[5:7])
                day = int(date[8:11])
            return (True, custom_strftime('%B {S}, %Y', date(year, month, day)), '')

        elif self.data['semesters'][-1]['semesterInfo']['expected_graduation_date']:
            date = self.data['semesters'][-1]['semesterInfo']['expected_graduation_date']
            if len(date) > 9:
                year = int(date[:4])
                month = int(date[5:7])
                day = int(date[8:11])
