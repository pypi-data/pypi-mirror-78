...
def getModule(moduleName):
    ...
    print(__modulesLoadesThreadNames__)
    ...
    for m in __modules_loaded__:
        if moduleName in m:
            ...
            __modulesLoadesThreadNames__.remove(moduleName)
    ...
    # Prevents any module not been fully loaded yet
    if moduleName in __modulesLoadesThreadNames__:
        for worker in getWorkers():
            if moduleName in worker:
                __sleep(0.1)
            ...
            for files in modules[modu][submod]:
                try:
                    ...
                    if not __amidjango__:
                        __modulesLoadesThreadNames__.append(module_name)
