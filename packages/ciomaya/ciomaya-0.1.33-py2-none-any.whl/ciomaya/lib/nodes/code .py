...
class conductorRender(om.MPxNode):
    ...
    def postConstructor(self):
        ...
        # TEMP - DONT DETECT HOST AND PLUGINS
    ...
    def detect_plugins(cls, fn_node):
        """Detect ONE renderer only, and only if nothing already on the plug.
        """
        print "DETECT PLUGINS"
        renderer_version = software.detect_mtoa() or software.detect_rfm() or software.detect_vray() or software.detect_redshift()
        if renderer_version:
            plug =  fn_node.findPlug(cls.aPluginSoftware, True)
            if plug.numElements() == 0:
                plug.elementByLogicalIndex( 0).setString(renderer_version)
    ...
    def set_default_metadata(cls, fn_node):
        element_plug = fn_node.findPlug(
            cls.aMetadata, True).elementByLogicalIndex(0)
