...
class Schema(MysqlModel):
    ...
    def add_params(self, names):
        ...
        for option in names:
            if option not in columns and 'dbtype' not in names[option]:
                self._create_column(option)
            elif option not in columns and 'dbtype' in names[option]:
                self._create_column(option, names[option]['dbtype'])
            self.parameters[option] = names[option]
...
class LiteSchema(SqLiteModel):
    ...
    def init_schema(self, method, params):
        if len(method) != 0:
            ...
            self.populate_new({'ID': create['ID']})
    ...
    def populate_new(self, *args, **kwargs):
