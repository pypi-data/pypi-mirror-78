...
class ParameterRoot(ParameterGroup):
    ...
    def __init__(self, parameter_data=None, warn=True,
                 **kwargs):
        self._warn = warn
        ...
        ParameterBuilder(self, warn=warn).build()
...
class ParameterBuilder(object):
    """ParameterBuilder"""
    def __init__(self, parameter_group, warn=True):
    ...
    def _factory(self, name, value_dict):
        ...
        if ptype == "group":
            ...
            ParameterBuilder(new_group, self._warn).build()
        elif ptype == "page":
            new_page = self._parameter_group.create_page(name)
            # Build groups recursively
            ParameterBuilder(new_page, self._warn).build()
        elif ptype == "integer":
