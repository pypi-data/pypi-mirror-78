...
class FlowIOInfo(QtWidgets.QDialog):
    ...
    def __init__(self, flowio_model, parent=None, flags=QtCore.Qt.Widget):
        ...
        editable_optional = (
            self._model.can_set_optional() and
            self._model.has_parent_port)

        self._default_edit.setChecked(self._model.default or False)
        self._default_edit.setEnabled(self._model.is_optional() and
                                      self._model.is_linked)
    ...
    def _set_optional_state(self, value):
        if self._model.is_linked:
            disabled = value == QtCore.Qt.Unchecked
            self._default_edit.setDisabled(disabled)
            if disabled:
                self._default_edit.setChecked(False)
