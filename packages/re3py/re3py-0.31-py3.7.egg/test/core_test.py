...
from learners.core.heuristic import HeuristicGini
...
class CoreTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.father = Relation("fatherOf", None, "test_data/fatherOf.txt",
                              ["Person", "Person"])
        cls.age = Relation("age", None, "test_data/age.txt",
                           ["Person", "numeric"])
        cls.friend = Relation("friend", None, "test_data/friend.txt",
                              ["Person", "Person"])
    ...
    def test_find_nominal_const_class(self):
        ...
        for target_data in [self.data_const1, self.data_const2]:
            ...
            score, comparator, subset, partition, _ = tree.find_best_nominal(
                tree_node, xs, target_data, [], TYPE_NOMINAL)
    ...
    def test_find_nominal_useless_splits_not_constant_class(self):
        ...
        xs = ["0", "1", "1", "1", "0", "0"]
        tree = DecisionTree(HeuristicGini(), stats)
        score, comparator, subset, partition, no_vars = tree.find_best_nominal(
            tree_node, xs, target_data, [], TYPE_NOMINAL)
    ...
    def test_find_nominal_perfect(self):
        ...
        xs = ["0", "3", "4", "5", "1", "2"]
        tree = DecisionTree(HeuristicGini(), stats)
        score, comparator, subset, partition, has_no_variables = tree.find_best_nominal(
            tree_node, xs, target_data, [], TYPE_NOMINAL, float('inf'))
    ...
    def test_find_nominal_with_target_variables(self):
        ...
        tree = DecisionTree(HeuristicGini(), stats)
        score, comparator, subset, partition, _ = tree.find_best_nominal(
            tree_node, xs, target_data, ["X1"], TYPE_TYPE)
        ...
        tree = DecisionTree(HeuristicGini(), stats)
        score, comparator, subset, partition, _ = tree.find_best_nominal(
            tree_node, xs, target_data, ["X2", "X0"], TYPE_TYPE)
    ...
    def test_find_numeric_const_class(self):
        ...
        for target_data, correct_score in zip(
            [self.data_const1, self.data_const2], correct_scores):
            ...
            tree = DecisionTree(HeuristicGini(), stats)
            score, comparator, threshold, partition = tree.find_best_numeric(
                tree_node, xs, target_data)
            self.assertEquals(score, correct_score)
    ...
    def test_memo_keys(self):
        ...
        relation_chain = [(self.father, ["Y1", "X1"]), (self.age, ["Y1",
                                                                   "Y1"]),
                          (self.father, ["X2", "Y2"])]
        ...
        solutions = [((('fatherOf', (False, False), (0, 'X1')), ),
                      [('mean', )], [[[1], [0]]]),
                     ((('fatherOf', (False, False), (0, 'X1')),
                       ('age', (False, False), (0, 0))), [('mean', 'min')],
                      [[[1], [0]], [[0, 1], []]]),
                     ((('fatherOf', (False, False),
                        (0, 'X1')), ('age', (False, False), (0, 0)),
                       ('fatherOf', (False, False), ('X2', 1))),
                      [('mean', 'min', 'max')], [[[1], [0]], [[0, 1], []],
                                                 [[0], [1]]])]
        for i in range(3):
            memo_keys = DecisionTree.test_values_memo_keys(
                example, relation_chain[:i + 1], [a_chains[i]])
            self.assertEqual(memo_keys, solutions[i])

        relation_chain = [(self.father, ["X1", "Y1"]), (self.age, ["Y2",
                                                                   "Y1"]),
                          (self.father, ["X2", "Y2"]), (self.age, ["Y1",
                                                                   "C3"])]
        a_chains = [(MEAN, MEAN, MIN, MIN), (MODE, MAX, MODE, MAX)]
        solution = ((('fatherOf', (False, False),
                      ('X1', 0)), ('age', (False, False), (1, 0)),
                     ('fatherOf', (False, False),
                      ('X2', 1)), ('age', (False, True), (0, 'cool'))),
                    [('mean', 'mean', 'min', 'min'),
                     ('mode', 'max', 'mode', 'max')], [[[0], [1]], [[1], [0]],
                                                       [[0, 1], []],
                                                       [[0, 1], []]])
        memo_keys = DecisionTree.test_values_memo_keys(example, relation_chain,
                                                       a_chains)
    ...
    def test_aggregator_chain_empty(self):
        ...
        chains1 = list(
            t.generate_possible_aggregator_chains(1,
                                                  [("var name", "numeric1")],
                                                  [123]))
        chains2 = list(
            t.generate_possible_aggregator_chains(2,
                                                  [("var name", "nominal3")],
                                                  [123]))
        chains3 = list(
            t.generate_possible_aggregator_chains(3,
                                                  [("var name", "nominal2")],
                                                  [123]))
    ...
    def test_aggregator_chain_one_fresh(self):
        ...
        self.assertListEqual([(c, TYPE_NUMERIC) for c in solution1],
                             sorted(chains1))
        self.assertListEqual([(c, TYPE_NUMERIC) if c[0] != MODE else
                              (c, "nominal3") for c in solution2],
                             sorted(chains2))
    ...
    def test_aggregator_chain_one_fresh_ends_with_flat(self):
        t = DecisionTree(allowed_aggregators=[
            "flatten", "flattenUnique", "count", "countUnique", "min", "max",
            "mean", "sum", "mode"
        ])
        chains1 = list(
            t.generate_possible_aggregator_chains(3,
                                                  ...
                                                  [0]))
        chains2 = list(
            t.generate_possible_aggregator_chains(3, [("var name", "nominal")],
                                                  [78]))
        chains3 = list(
            t.generate_possible_aggregator_chains(3, [("var name", "Horse")],
                                                  [21]))
        end_with_flatten = [[COUNT, FLATTEN, FLATTEN],
                            [COUNT_UNIQUE, FLATTEN, FLATTEN],
                            [MIN, FLATTEN, FLATTEN], [MAX, FLATTEN, FLATTEN],
                            [MEAN, FLATTEN, FLATTEN], [SUM, FLATTEN, FLATTEN],
                            [COUNT, FLATTEN_UNIQUE, FLATTEN],
                            [COUNT_UNIQUE, FLATTEN_UNIQUE, FLATTEN],
                            [MIN, FLATTEN_UNIQUE, FLATTEN],
                            [MAX, FLATTEN_UNIQUE, FLATTEN],
                            [MEAN, FLATTEN_UNIQUE, FLATTEN],
                            [SUM, FLATTEN_UNIQUE, FLATTEN],
                            [MIN, COUNT, FLATTEN], [MAX, COUNT, FLATTEN],
                            [MEAN, COUNT, FLATTEN], [SUM, COUNT, FLATTEN],
                            [MIN, COUNT_UNIQUE, FLATTEN],
                            [MAX, COUNT_UNIQUE, FLATTEN],
                            [MEAN, COUNT_UNIQUE, FLATTEN],
                            [SUM, COUNT_UNIQUE, FLATTEN], [MIN, MIN, FLATTEN],
                            [MAX, MIN, FLATTEN], [MEAN, MIN, FLATTEN],
                            [SUM, MIN, FLATTEN], [MIN, MAX, FLATTEN],
                            [MAX, MAX, FLATTEN], [MEAN, MAX, FLATTEN],
                            [SUM, MAX, FLATTEN], [MIN, MEAN, FLATTEN],
                            [MAX, MEAN, FLATTEN], [MEAN, MEAN, FLATTEN],
                            [SUM, MEAN, FLATTEN], [MIN, SUM, FLATTEN],
                            [MAX, SUM, FLATTEN], [MEAN, SUM, FLATTEN],
                            [SUM, SUM, FLATTEN]]
        end_with_flatten_unique = [
            chain[:2] + [FLATTEN_UNIQUE] for chain in end_with_flatten
        ]
        solution1 = sorted(end_with_flatten + end_with_flatten_unique)
        solution1 = [(x, TYPE_NUMERIC) for x in solution1]
        answer = sorted(
            [c for c in chains1 if c[0][-1] in [FLATTEN, FLATTEN_UNIQUE]])
        self.assertListEqual(solution1, answer)
        solution2 = [[MODE, FLATTEN, FLATTEN], [MODE, FLATTEN_UNIQUE, FLATTEN],
                     [COUNT, FLATTEN, FLATTEN],
                     [COUNT, FLATTEN_UNIQUE, FLATTEN],
                     [COUNT_UNIQUE, FLATTEN, FLATTEN],
                     [COUNT_UNIQUE, FLATTEN_UNIQUE, FLATTEN],
                     [MODE, MODE, FLATTEN], [COUNT, MODE, FLATTEN],
                     [COUNT_UNIQUE, MODE, FLATTEN], [MIN, COUNT, FLATTEN],
                     [MAX, COUNT, FLATTEN], [MEAN, COUNT, FLATTEN],
                     [SUM, COUNT, FLATTEN], [MIN, COUNT_UNIQUE, FLATTEN],
                     [MAX, COUNT_UNIQUE,
                      FLATTEN], [MEAN, COUNT_UNIQUE, FLATTEN],
                     [SUM, COUNT_UNIQUE, FLATTEN]]
        solution2 += [chain[:2] + [FLATTEN_UNIQUE] for chain in solution2]
        solution2 += [[MODE, MODE, MODE], [COUNT, MODE, MODE],
                      [COUNT_UNIQUE, MODE, MODE], [MODE, FLATTEN, MODE],
                      [COUNT, FLATTEN, MODE], [COUNT_UNIQUE, FLATTEN, MODE],
                      [MODE, FLATTEN_UNIQUE, MODE],
                      [COUNT, FLATTEN_UNIQUE, MODE],
                      [COUNT_UNIQUE, FLATTEN_UNIQUE, MODE], [MIN, COUNT, MODE],
                      [MAX, COUNT, MODE], [MEAN, COUNT, MODE],
                      [SUM, COUNT, MODE], [MIN, COUNT_UNIQUE, MODE],
                      [MAX, COUNT_UNIQUE, MODE], [MEAN, COUNT_UNIQUE, MODE],
                      [SUM, COUNT_UNIQUE, MODE]]
        package = [MAX, MIN, MEAN, SUM]
        for a in [MAX, MIN, MEAN, SUM]:
            for b in [COUNT, COUNT_UNIQUE]:
                solution2 += [[x, a, b] for x in package]
        types = [
            TYPE_NUMERIC if chain[0] != MODE else "nominal"
            for chain in solution2
        ]
        solution3 = solution2
        solution2 = [(c, t) for c, t in zip(solution2, types)]
        self.assertListEqual(sorted(solution2), sorted(chains2))
        types = [
            TYPE_NUMERIC if chain[0] != MODE else "Horse"
            for chain in solution3
        ]
    ...
    def test_aggregator_chain_no_fresh(self):
        ...
        t = DecisionTree(
            allowed_aggregators=["count", "sum", "mode", "flatten"])
