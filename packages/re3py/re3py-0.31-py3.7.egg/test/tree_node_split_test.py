...
from learners.core.aggregators import *
...
class SplitTest(unittest.TestCase):
    ...
    def test_first_no_hits_simple(self):
        ...
        tests = [[self.age, ["Y1", "X1"], MIN],
                 [self.age, ["Y2", "X1"], COUNT],
                 [self.age, ["Y1", "X1"], COUNT]]
        ths = [float("inf"), 0, 1]
    ...
    def test_then_no_hits_long(self):
        ...
        b1 = VariableVariable("X1", "Person", "Jim7")
        example = {"Y1": f1, "X1": b1, "Y2": f2}
        tests = [(self.father, ["Y1", "X1"], MIN), (self.age, ["Y1",
                                                               "Y2"], MIN)]
    ...
    def test_ages_of_grandparents_more_generators(self):
        ...
        chains_relations = [(self.father, ["Y1", "X1"]),
                            (self.father, ["Y2", "Y1"]),
                            (self.age, ["Y2", "Y3"])]
        chains_aggregators = [(MEAN, FLATTEN_UNIQUE, MIN),
                              (MEAN, FLATTEN, MIN)]
        eps = 10**-9
        s = BinarySplit([], None, None, True, True)
        answer = s.evaluate_all(example, chains_relations, chains_aggregators,
                                [SMALLER, SMALLER], [16 + eps, 49 / 3 + eps])
        self.assertListEqual([True, True], answer)
        answer = s.evaluate_all(example, chains_relations, chains_aggregators,
                                [BIGGER, BIGGER], [16 - eps, 49 / 3 - eps])
        self.assertListEqual([True, True], answer)
