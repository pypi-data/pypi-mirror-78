import unittest
from learners.core import aggregators
...
class AggTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.tasks = [[], [1], [1, 2, 3], [[]], [[], []], [[1, 2, 3]],
                     [[1, 2, 2], [1, 2, 8, 1], [1, 2, 2]]]
        cls.flatten_sol = [[], None, None, [], [], [1, 2, 3],
                           [1, 2, 2, 1, 2, 8, 1, 1, 2, 2]]
        cls.flatten_sol_flat = [[], [1], [1, 2, 3], [[]], [[], []], [[1, 2,
                                                                      3]],
                                [[1, 2, 2], [1, 2, 8, 1], [1, 2, 2]]]
        ...
        cls.min_sol = [[], None, None, [float("inf")],
                       [float("inf"), float("inf")], [1], [1, 1, 1]]
        cls.min_sol_flat = [float("inf"), 1, 1, None, None, None, None]
        cls.max_sol = [[], None, None, [float("-inf")],
                       [float("-inf"), float("-inf")], [3], [2, 8, 2]]
        cls.max_sol_flat = [float("-inf"), 1, 3, None, None, None, None]
        cls.mean_sol = [[], None, None, [float("inf")],
                        [float("inf"), float("inf")], [2], [5 / 3, 3, 5 / 3]]
        ...
        cls.mode_sol = [[], None, None, [aggregators.MODE_OF_EMPTY_LIST],
                        [
                            aggregators.MODE_OF_EMPTY_LIST,
                            aggregators.MODE_OF_EMPTY_LIST
                        ], [1], [2, 1, 2]]
        cls.mode_sol_flat = [
            aggregators.MODE_OF_EMPTY_LIST, 1, 1, [], [], [1, 2, 3], [1, 2, 2]
        ]
    ...
    def test_flatten(self):
        for t, s, f_s in self.solution_generator(self.flatten_unique_sol,
                                                 self.flatten_unique_sol_flat):
            if s is not None:
                self.assertListEqual(
                    sorted(aggregators.FLATTEN_UNIQUE.aggregate(t)), sorted(s))
            self.assertListEqual(aggregators.FLATTEN_UNIQUE.aggregate_flat(t),
                                 f_s)

    def test_flatten_unique(self):
        for t, s, f_s in self.solution_generator(self.mode_sol,
                                                 self.mode_sol_flat):
            if s is not None:
    ...
    def test_count(self):
        for t, s, f_s in self.solution_generator(self.count_sol,
                                                 self.count_sol_flat):
            if s is not None:
    ...
    def test_count_unique(self):
        for t, s, f_s in self.solution_generator(self.count_unique_sol,
                                                 self.count_unique_sol_flat):
            if s is not None:
                a = aggregators.COUNT_UNIQUE.aggregate(t)
                self.assertListEqual(a, s)
            a = aggregators.COUNT_UNIQUE.aggregate_flat(
                [tuple(w) if type(w) == list else w for w in t])
            self.assertEqual(a, f_s)

    def test_min(self):
        for t, s, f_s in self.solution_generator(self.min_sol,
                                                 self.min_sol_flat):
            if s is not None:
    ...
    def test_max(self):
        for t, s, f_s in self.solution_generator(self.max_sol,
                                                 self.max_sol_flat):
            if s is not None:
    ...
    def test_mean(self):
        for t, s, f_s in self.solution_generator(self.mean_sol,
                                                 self.mean_sol_flat):
            if s is not None:
    ...
    def test_sum(self):
        for t, s, f_s in self.solution_generator(self.sum_sol,
                                                 self.sum_sol_flat):
            if s is not None:
