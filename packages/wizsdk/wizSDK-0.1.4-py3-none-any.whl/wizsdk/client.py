...
class Client(DeviceContext, Keyboard):
    ...
    async def move_mouse(self, x, y, duration=0.5):
        """ Moves to mouse to the position (x, y) relative to the window's position """
        wx, wy = self.get_rect()[:2]
        await mouse.move_to(wx + x, wy + y, duration=duration)
        return self

    async def click(self, x, y, delay=0.1, duration=0.5, button="left"):
        """ Moves the mouse to (x, y) relative to the window and presses the mouse button """
        self.set_active()
        await self.move_mouse(x, y, duration=duration)
