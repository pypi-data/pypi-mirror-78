...
class ItemMonthSummary(SalesInvBase):
    ...
    def PublishRefreshLists(cls, itemType, startMonth=None,doPublish=True):
        ...
        for key in ia_items.keys():
            ...
            publish_lists.append(cls.PublishRefreshListsName(itemType,ia_item.name,startMonth,doPublish))
    ...
    def PublishRefreshListsName(cls, itemType, itemName, startMonth=None, doPublish=True):
        ...
        if doPublish:
            cls._publishRefreshListsName(nameRefreshList)
        return nameRefreshList
    
    @classmethod
    def _publishRefreshListsName(cls, refreshList):
        qsKey = cls._createQsKey(refreshList['item_type'],refreshList['item_name'])
        ...
        if qsVal is None:
            ...
            pubList = {"item_name":refreshList['item_name'],'item_type':refreshList['item_type'],'grow_months':strGrowMonths}
            publish_message(UPDATE_MONTH_INVENTORY,'Start Processing Name Refresh',pubList)
        else:
            print("This item combo (itemType: {}, itemName: {}) is in the middle of a refresh.. will try later..".format(refreshList['item_type'],refreshList['item_name']))
    ...
    @classmethod
    def _createQsKey(cls,itemType, itemName):
        return itemType+"_"+itemName+"_refresh"
    ...
    def FinishPublishChain(cls,publishInfo,doPublish=True):
        ...
        if 'item_type' in keys and 'item_name' in keys:
             qsKey = cls._createQsKey(publishInfo['item_type'],publishInfo['item_name'])
             qsVal = QuickStorage.getValue(qsKey)
             if qsVal is not None:
                 ...
                 if len(added_months) > 0:
                     ...
                     cls.PublishRefreshListsName(publishInfo['item_type'], publishInfo['item_name'], startMonth, doPublish)
    ...
    def PopProcessPublish(cls,publishInfo,doPublish=True):
        ...
        try:
            ...
            print("Not able to pull Item Month ({}) for item: {} - {}".format(growMonth,itemType,itemName))
            return None
