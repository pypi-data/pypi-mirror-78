...
class ComparisionTimeval(object):
    OP_EQ = ""
    OP_GTE = "gte"
    OP_LTE = "lte"
    OP_LT = "lt"
    ...
    def __eq__(self, other):
        ...
        v1 = self.time_val
        v2 = dt
        if isinstance(dt, ComparisionTimeval):
            v2 = dt.time_val

        val = func(v2, v1)

        if isinstance(other, ComparisionTimeval):
    ...
    def create_timeval(val):
        if isinstance(val, StringTypes):
            pat = re.compile(r'{time\.(' + "|".join(ComparisionTimeval.ALL_OPS) + r')now\(\)\:(.+)}')
            match = pat.match(val)
            if match:
...
class QueryStringParser(object):
    ...
    def is_datetime(val):
        ...
        """
        try:
            return ComparisionTimeval.create_timeval(val)
