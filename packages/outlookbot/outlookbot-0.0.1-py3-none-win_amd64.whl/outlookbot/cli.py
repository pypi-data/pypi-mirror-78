...
@click.option("--attachment-dir", "-a", default=".", 
              help="The directory from which attachment files would be picked")
...
@click.option("--dry-run", "-d", is_flag=True, default=False, show_default=True, 
              help="Dry run")
def fire(config_file, email_data_file, template_file, attachment_dir, format, dry_run):
    """
        Send email
    """
    ...
    send_email(email_data_file, template_file, attachment_dir, format, dry_run)
