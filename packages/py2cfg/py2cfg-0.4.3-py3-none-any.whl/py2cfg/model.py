...
class Block(object):
    ...
    def type(self, default=None):
        default = ast.Pass if default is None else default
...
class CFG(object):
    ...
    def _style_handler(self, block, typeobject):
    ...
    def _edge_handler(self, link):
        return None, "black", link.get_exitcase().strip()

    def stylize_node(self, block: Block, default=None) -> tuple:
        ...
        if calls and block.func_calls:
            ...
            for line in calls_label.splitlines():
                if "input" in line:
                    ...
                    tmp += line
            calls_label = tmp
            shape, color = self.node_styles[ast.Call]
            graph.node(
                calls_node,
                label=calls_label,
                _attributes={
