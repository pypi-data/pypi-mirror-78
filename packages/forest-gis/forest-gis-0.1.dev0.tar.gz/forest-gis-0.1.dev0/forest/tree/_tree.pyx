...
cdef class Tree:
    ...
    cpdef compute_feature_importances(self, normalize=True):
        ...
        return importances
    cpdef  DOUBLE_t[:,:] local_variable_importance_g(self, object X, object y, object grouping, object subs_num):
        return self._parse(X, y, grouping, subs_num)

    cdef inline DOUBLE_t[:,:] _parse(self, object X, np.ndarray y, np.ndarray grouping, SIZE_t subs_num):
        # Check input
        if X.dtype != DTYPE:
            raise ValueError("X.dtype should be np.float32, got %s" % X.dtype)
        # Extract input
        cdef DTYPE_t[:, :] X_ndarray = X
        cdef DOUBLE_t[:] y_ndarray = y
        cdef INT32_t[:,:] group = grouping
        cdef DOUBLE_t[:,:] importances
        #cdef SIZE_t subs_num = group.shape[0]
        importances = np.zeros((subs_num, self.n_features))
        cdef SIZE_t samples_num = X_ndarray.shape[0]
        cdef SIZE_t subs_i = 0
        cdef SIZE_t i = 0
        cdef SIZE_t j
        ## refering to the struct
        cdef SIZE_t node_num = self.node_count
        cdef Node* node = NULL
        cdef Node* left
        cdef Node* right
        cdef Node* nodes = self.nodes
            ...
            for subs_i in range(subs_num):
                for i in range(samples_num):
                    if group[subs_i, i] != 0:
                        ...
                        while node.left_child != _TREE_LEAF:
                            # ... and node.right_child != _TREE_LEAF:
                            node.local_y_sum += y_ndarray[i]
                            node.local_squared_y_sum += (y_ndarray[i])**2
                            node.local_case_num += 1
                            if X_ndarray[i, node.feature] <= node.threshold:
                                node = &self.nodes[node.left_child]
                            else:
                                node = &self.nodes[node.right_child]
                ...
                node = self.nodes
                node.local_impurity = node.local_squared_y_sum - node.local_y_sum**2/node.local_case_num
                node.local_case_num = 0
                node.local_y_sum = 0
                node.local_squared_y_sum = 0
                for j in range(node_num-1):
                    if node.left_child != _TREE_LEAF:
                        # ... and node.right_child != _TREE_LEAF
                        if node.local_case_num > 2:
                            left = &nodes[node.left_child]
                            left.local_impurity = left.local_squared_y_sum - left.local_y_sum**2 / left.local_case_num
                            right = &nodes[node.right_child]
                            right.local_impurity = right.local_squared_y_sum - right.local_y_sum**2 / right.local_case_num
                            node.local_case_num = 0
                            node.local_y_sum = 0
                            node.local_squared_y_sum = 0
                            importances[subs_i, node.feature] += (node.local_impurity - left.local_impurity - right.local_impurity)
                        if node.local_case_num > 0:
                            node.local_case_num = 0
                            node.local_y_sum = 0
                            node.local_squared_y_sum = 0
                    node += 1
