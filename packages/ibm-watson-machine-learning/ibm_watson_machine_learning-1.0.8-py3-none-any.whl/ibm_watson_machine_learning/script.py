...
class Script(WMLResource):
    ...
    def store(self, meta_props, file_path):
        ...
        response = self._create_asset(script_meta, file_path)
    ...
    def delete(self, asset_uid):
        ...
        if not self._ICP:
            if self._client.CLOUD_PLATFORM_SPACES and self._if_deployment_exist_for_asset(asset_uid):
                raise WMLClientError(
    ...
    def _get_required_element_from_response(self, response_data):
        ...
        try:
            if self._client.default_space_id is not None:
                ...
                if self._client.ICP_30 is not None or self._client.CLOUD_PLATFORM_SPACES:
