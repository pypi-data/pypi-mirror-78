...
#country= us
def pull_finData(period,country):
    sf.set_api_key('free')
    sf.set_data_dir('~/simfin_data/')
    df_companies = sf.load_companies(market=country)
    df_industries = sf.load_industries()
    df_income = sf.load_income(variant=period, market=country)
    df_balance = sf.load_balance(variant=period, market=country)
    df_cashflow = sf.load_cashflow(variant=period, market=country)
