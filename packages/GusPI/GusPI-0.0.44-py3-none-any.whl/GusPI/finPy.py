...
#country= us
def prepare_finData(period,country):
    sf.set_api_key('free')
    sf.set_data_dir('~/simfin_data/')
    df_companies = sf.load_companies(market='us')
    df_industries = sf.load_industries()
    
def pull_finData_income(period,country):
    prepare_finData(period,country)
    df_income = sf.load_income(variant=period, market='us')
    return df_income

def pull_finData_balance(period,country):
    prepare_finData(period,country)
    df_balance = sf.load_balance(variant=period, market='us')
    return df_balance

def pull_finData_cashflow(period,country):
    prepare_finData(period,country)
    df_cashflow = sf.load_cashflow(variant=period, market='us')
    return df_cashflow
