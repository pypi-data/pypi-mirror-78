...
from .parsers import AddressParser, ContactEmailNamePairParser, ContactEmailParser, ContactPhoneParser
...
class Document:
    ...
    FACT_NAME_PHONE = "PHONE"
    FACT_NAMES_NER = ("PER", "ORG", "GPE", "LOC")
    FACT_NAME_NAMEMAIL = "NAMEMAIL"

    KNOWN_ENTITIES = (FACT_NAME_EMAIL, FACT_NAME_ADDRESS, FACT_NAME_PHONE, FACT_NAME_NAMEMAIL) + FACT_NAMES_NER
    ...
    def contacts(self):
        ...
        phone_numbers = ContactPhoneParser(text, months=self.concat_resources["months"]).parse()
        self.__texta_facts.extend((number.to_fact(Document.FACT_NAME_PHONE, self.doc_path) for number in phone_numbers))
