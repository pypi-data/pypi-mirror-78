...
__all__ = [	'chamfer', 'bevel', 'beveltgt', 
			'tangentjunction', 'cut', 'cutsegments', 'cutoffsets',
...
def chamfer(mesh, line, cutter):
	''' create a chamfer on the given suite of points, create faces are planes.
		cutter is described in function cutoffsets()
	'''
	# cut faces
	conn = connef(mesh)
	segments = cut(mesh, line, cutoffsets(mesh, line, cutter, conn), conn)
...
def bevel3(mesh, line, cutter, interpol=spline, resolution=None):
	...
	segments = cut(mesh, line, cutoffsets(mesh, line, cutter), conn)
	conn = connef(mesh.faces)
...
def cutoffsets(mesh, edges, cutter, conn=None):
	...
	offsets = []
	for e in edges:
        fn1 = mesh.facenormals(conn[e])
        fn2 = mesh.facenormals(conn[(e[1], e[0])])
...
def cut(mesh, edges, offsets, conn=None):
    match = []
    offsets = dict(zip(edges, offsets))
    for wire in suites(edges):
        match += cutcontiguous(mesh, wire, [offsets[(wire[i-1], wire[i])] for i in range(1,len(wire))], conn)
        
    # shared points in the wire, rectifie matches to not overpass their intersection points
    indev

def cutcontiguous(mesh, line, offsets, conn=None):
	...
	match = []				# intersection segments for each offset
	prec = mesh.precision()
	# build connectivity
	if not conn:
		conn = connef(mesh.faces)
...
def faceheight(mesh, fi):
	...
	return min(heights)    # TODO prender en compte les NAN
