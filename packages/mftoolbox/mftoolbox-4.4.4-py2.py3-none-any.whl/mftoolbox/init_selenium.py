...
def ClearCache():
    ...
    #browser.set_window_position(browser_window_position['x'], browser_window_position['y'])
    except not Sair:
        print('Não foi possível criar a instance dp browser')
    finally:
        browser.close()

def InitBrowser(headless=True):
    browser.close()
    if headless = True
        __options.add_argument("headless")
    # noinspection SpellCheckingInspection
