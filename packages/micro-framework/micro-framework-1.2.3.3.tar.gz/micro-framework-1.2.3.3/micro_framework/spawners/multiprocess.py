...
def process_worker(call_queue, result_queue, max_tasks):
    ...
    while True:
        try:
            ...
            logger.error(f"Worker {os.getpid()} Received message")
        except Empty:
            continue
        ...
        try:
            logger.error(f"Worker [PID={os.getpid()}] is starting task "
                         f"{call_item.task_id}.")
            result = function(*fn_args, **fn_kwargs)
        except Exception as _exc:
            exc = _ExceptionWithTraceback(_exc, _exc.__traceback__)
        logger.error(f'Worker {os.getpid()} finished message handling')
        ...
        if max_tasks and executed_tasks >= max_tasks:
            ...
            logger.error(f"Worker {os.getpid()} reached its max_tasks")
            return
...
class ProcessSpawner(Spawner, _base.Executor):
    ...
    def submit(self, target_fn, *args, **kwargs):
        ...
        logger.error("Sending task to write queue")
    ...
    def _send_task_to_process(self):
        while True:
            try:
                ...
                logger.error("Write queue received task")
            except Empty:
                return
            logger.error("Sending task to call queue")
            self.call_queue.put(
    ...
    def _read_task_result(self):
        try:
            ...
            logger.error("Received Read message")
        except Empty:
            return False
        if result is None:
            logger.error("Received Message is None")
            return False
        logger.error("Received Message is a worker response")
    ...
    def _check_pool(self):
        ...
        with self.shutdown_lock:
            if self.shutdown_signal:
                return
        logger.error(f"Checking for dead processes in the pool. Currently "
...
class ProcessPoolSpawner(Spawner, ProcessPoolExecutor):
    ...
    def shutdown(self, wait: bool = ...) -> None:
        logger.error("Greedy Worker shutdown initiated, wait until all pending "
                    "tasks are consumed.")
