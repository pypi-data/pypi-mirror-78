...
"""
DATAFILE = r"macrodemos\data\IFS_GDP.xlsx"
...
"""
def HP(x: pd.DataFrame, lambda_: float):
    """
...
def BKcycle(x: pd.DataFrame, K:int=8, pL=6, pH=32):
    """
    Baxter-King filter
    ...
    h = np.arange(-K, K+1)  # lags
    beta = (np.sin(h*wh) - np.sin(h*wl)) / (h*np.pi)
    beta[K] = (wh-wl)/np.pi # fix division by zero
...
def CFcycle(x: pd.DataFrame, pL=6, pH=32):
    """
    ...
    cycle, trend = cffilter(np.log(x), pL, pH, drift=False)
    cycle.columns = x.columns
    trend.columns = x.columns

    return np.exp(trend), 100*cycle


def Hamilton(y: pd.DataFrame, h=8, p=4):
    """
...
def app_choose_parameter(label, appid, list_of_options, value):
    ...
    return html.Tr([html.Th(label), html.Th(dcc.Dropdown(id=appid, options=options, value=value),style={'width': '100%'})])

def parse_parameters(lambda_, K, pL, pH, cfpL, cfpH, h, p):
...
def update_plots(n_clicks, indicator, whichFilters, lambda_, K, pL, pH, cfpL, cfpH, h, p, country):
    params = parse_parameters(lambda_, K, pL, pH, cfpL, cfpH, h, p)
    trend = go.Figure()
    cycle = go.Figure()
    ...
    cycle.update_layout(title_text=f'Cycle: {indicator} in {country}', xaxis_rangeslider_visible=True, height=700)
...
def filters_demo(colab=True):

    print('NOT READY: THIS DEMO WILL BE READY FOR NEXT RELEASE')
    return None
