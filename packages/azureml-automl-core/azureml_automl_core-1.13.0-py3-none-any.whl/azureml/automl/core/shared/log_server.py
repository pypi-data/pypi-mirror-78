...
def _wait_for_log(logger_name: str, timeout: Optional[float] = 10) -> None:
    """
    ...
    with cv:
        ...
        success = cv.wait(timeout=timeout)
        ...
        assert success, "Waited {} seconds for a log event on {} but no logs were emitted.".format(
            timeout, logger_name
        )
