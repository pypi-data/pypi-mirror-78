...
def automl_train(train_csv,model_dir,mode):
    ...
    model.fit(df_X.astype('int'), df_y.astype('int'))
    ...
    if not is_big:
        for i, (train, test) in enumerate(kf.split(df_X)):
            ...
            model.fit(train_x.astype('int'), train_y.astype('int'), eval_set=(test_x.astype('int'), test_y.), early_stopping_rounds=7)
            models.append(model)
        ...
        if TIME_LIMIT > 5 * 60:
            ...
            model.fit(train_x.astype('int'), train_y.astype('int'), eval_set=(test_x, test_y), early_stopping_rounds=7)
        else:
            if mode == 'regression':
