...
class AbstractExProtoHandler(metaclass=ABCMeta):
    ...
    def send(self, connection: Connection, data: bytes):
        erlang.call(Atom(b'emqx_exproto'), Atom(b'send'), (connection.pid, data), 5000)

    def terminate(self, connection: Connection):
        erlang.call(Atom(b'emqx_exproto'), Atom(b'close'), (connection.pid), 5000)

    def register(self, connection: Connection, client_info: ClientInfo):
        erlang.call(Atom(b'emqx_exproto'), Atom(b'register'), (connection.pid, client_info.to_erlang_data_type()), 5000)

    def publish(self, connection: Connection, message: Message):
        erlang.call(Atom(b'emqx_exproto'), Atom(b'publish'), (connection.pid, message.to_erlang_data_type()), 5000)

    def subscribe(self, connection: Connection, topic: str, qos: int):
        erlang.call(Atom(b'emqx_exproto'), Atom(b'subscribe'), (connection.pid, bytes(topic), qos), 5000)