...
class NiuCloud:
    ...
    async def update_vehicles(self):
        ...
        for vehicle in vehicles:
            ...
            resp = await self._request("GET",
                                       NIU_API_URL +
                                       f"/v5/scooter/detail/{veh.serial_number}",
                                       )
    ...
    async def _request(self, method, url, data=None, params=None):
        async with aiohttp.ClientSession() as session:
            if method == 'GET':
                resp = await session.get(url=url, data=data, params=params, headers={**HTTP_HEADER, "token": self.token if not self.token is None else ""})
            elif method == 'POST':
                resp = await session.post(url=url, data=data, params=params, headers={**HTTP_HEADER, "token": self.token if not self.token is None else ""})
            else:
