...
class NiuCloud:
    ...
    async def _request(self, method, url, data=None, params=None):
        async with aiohttp.ClientSession() as session:
            if method == 'GET':
                resp = await session.get(url=url, data=data, params=params, headers={**HTTP_HEADER, "token": SESSION.token})
            elif method == 'POST':
                resp = await session.post(url=url, data=data, params=params, headers={**HTTP_HEADER, "token": SESSION.token})
            else:
