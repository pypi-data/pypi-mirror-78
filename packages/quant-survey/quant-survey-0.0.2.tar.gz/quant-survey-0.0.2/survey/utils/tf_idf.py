...
def plot_tf_idf_for_cat_text_question(survey: Survey,
                                      categorical_question: CategoricalQuestion, text_question: str):
    """
    Plot TF IDF wordcloud for each group with the same answer for a categorical question on a text question.
    ...
    for cat_value in categorical_question.categories:
        respondents = survey.find_respondents(question=categorical_question, answers=cat_value)
        responses = survey.question_responses(text_question, respondents)
        corpus.append(responses.str.cat(sep=' '))
    tf = TfidfVectorizer(analyzer='word', ngram_range=(1, 2), min_df=0, max_df=1, stop_words='english')
    doc_term_frequency = tf.fit_transform(corpus)
    features = tf.get_feature_names()
    for i in range(len(categorical_question.categories)):
        top_words = top_words_in_doc(doc_term_frequency=doc_term_frequency, features=features, row_id=i)
...
def plot_tf_idf_for_control_experiment_groups(
        ...
        exp_attribute_values: Union[str, List[str]], ctl_attribute_values: Union[str, List[str]],
        text_question: str
):
    """
    Plot TF IDF wordcloud of answers to a FreeTextQuestion by the Experimental Group vs Control Group, where:
        - each Group answered one of `cat_answers` to `cat_question`
        - the Experimental Group all have one of `exp_attribute_values` for `attribute`
        - the Control Group all have one of `ctl_attribute_values` for `attribute`
    ...
    :param attribute: Demographic attribute for which the Experimental and Control Groups have different values.
    ...
    exp_respondents = survey.find_respondents(cat_question, cat_answers, attribute, exp_attribute_values)
    ctl_respondents = survey.find_respondents(cat_question, cat_answers, attribute, ctl_attribute_values)
    ...
    tf = TfidfVectorizer(analyzer='word', ngram_range=(1, 2), max_df=1, stop_words='english')
    doc_term_frequency = tf.fit_transform(corpus)
    features = tf.get_feature_names()
    # create wordcloud for each group
    attribute_values = [listify(exp_attribute_values), listify(ctl_attribute_values)]
...
def plot_tf_idf_for_control_experiment_groups_attributes(
    ...
    """
    # get respondents experimental and control group respondents
    exp_respondents = survey.find_respondents(filter_category=attribute, filter_values=exp_attribute_values)
    ctl_respondents = survey.find_respondents(filter_category=attribute, filter_values=ctl_attribute_values)
    # create corpus of 2 documents, one for each group
    exp_responses = survey.question_responses(text_question, exp_respondents)
    ctl_responses = survey.question_responses(text_question, ctl_respondents)
    corpus = [exp_responses.str.cat(sep=' '), ctl_responses.str.cat(sep=' ')]
...
def plot_tf_idf_for_control_experiment_groups_diff_response(
        survey: Survey,
        cat_question: CategoricalQuestion, exp_cat_answers: List[str], ctl_cat_answers: List[str],
        attribute: SingleCategoryAttribute,
        exp_attribute_values: Union[str, List[str]], ctl_attribute_values: Union[str, List[str]],
        text_question: str
):
    ...
    :param exp_cat_answers: Answers given by experimental group to `cat_question`.
    ...
    """
    # get respondents experimental and control group respondents
    exp_respondents = survey.find_respondents(cat_question, exp_cat_answers, attribute, exp_attribute_values)
    ctl_respondents = survey.find_respondents(cat_question, ctl_cat_answers, attribute, ctl_attribute_values)
    # create corpus of 2 documents, one for each group
    exp_responses = survey.question_responses(text_question, exp_respondents)
    ctl_responses = survey.question_responses(text_question, ctl_respondents)
    corpus = [exp_responses.str.cat(sep=' '), ctl_responses.str.cat(sep=' ')]
    # fit TF IDF vectorizer and get matrix and feature names
    tf = TfidfVectorizer(analyzer='word', max_df=1, ngram_range=(1, 2), stop_words='english')
    doc_term_frequency = tf.fit_transform(corpus)
    features = tf.get_feature_names()
    # create wordcloud for each group
    attribute_values = [listify(exp_attribute_values), listify(ctl_attribute_values)]
