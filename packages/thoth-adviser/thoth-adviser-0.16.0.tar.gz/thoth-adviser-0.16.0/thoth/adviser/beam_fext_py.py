...
class Beam:
    ...
    def size(self) -> int:
        ...
        assert len(self._heapq) == len(self._heap)
    ...
    def iter_states(self) -> List[State]:
        ...
        assert self._heap.items() == self._heapq
        return self._heapq
    ...
    def max(self) -> State:
        ...
        try:
            assert self._heap.get_max() == self._max[0]
        except AssertionError:
            from pprint import pprint

            print(id(self._heap.get_max()), "==", id(self._max[0]))
            pprint(self._heap.get_max().to_dict())
            pprint(self._max[0].to_dict())
            raise
        return self._max[0]
    ...
    def pop(self, idx: Optional[int] = None) -> State:
        ...
        assert self._heap.get(idx) == removed
